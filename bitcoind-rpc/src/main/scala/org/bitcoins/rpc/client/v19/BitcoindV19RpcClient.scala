...
/** Class for creating a BitcoindV19 instance that can access RPCs
  */
class BitcoindV19RpcClient(override val instance: BitcoindInstance)(implicit actorSystem: ActorSystem)
...
object BitcoindV19RpcClient {
  ...
  /** Creates an RPC client from the given instance,
    ...
    * over the RPC client.
    */
  def withActorSystem(instance: BitcoindInstance)(implicit system: ActorSystem): BitcoindV19RpcClient =
    new BitcoindV19RpcClient(instance)(system)

  def fromUnknownVersion(rpcClient: BitcoindRpcClient): Try[BitcoindV19RpcClient] =
    Try {
