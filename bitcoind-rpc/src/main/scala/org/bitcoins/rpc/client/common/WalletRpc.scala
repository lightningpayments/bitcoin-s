...
import org.bitcoins.commons.jsonmodels.bitcoind.RpcOpts.{AddressType, WalletCreateFundedPsbtOptions, WalletFlag}
...
import org.bitcoins.crypto.{DoubleSha256Digest, DoubleSha256DigestBE, ECPrivateKeyBytes, ECPublicKey}
...
trait WalletRpc { self: Client =>
  ...
  def backupWallet(destination: String, walletNameOpt: Option[String] = None): Future[Unit] = {
    bitcoindCall[Unit]("backupwallet",
                       List(JsString(destination)),
                       uriExtensionOpt = walletNameOpt.map(walletExtension))
  }

  def dumpPrivKey(address: BitcoinAddress, walletNameOpt: Option[String] = None): Future[ECPrivateKeyBytes] = {
  ...
  def dumpWallet(filePath: String, walletNameOpt: Option[String] = None): Future[DumpWalletResult] = {
    bitcoindCall[DumpWalletResult]("dumpwallet",
                                   List(JsString(filePath)),
                                   uriExtensionOpt = walletNameOpt.map(walletExtension))
  }

  def encryptWallet(passphrase: String, walletNameOpt: Option[String] = None): Future[String] = {
  ...
  def getBalance(walletName: String): Future[Bitcoins] = {
    bitcoindCall[Bitcoins]("getbalance", uriExtensionOpt = Some(walletExtension(walletName)))
  }
  ...
  def getReceivedByAddress(
    ...
    bitcoindCall[Bitcoins]("getreceivedbyaddress",
  ...
  def getUnconfirmedBalance(walletName: String): Future[Bitcoins] = {
    bitcoindCall[Bitcoins]("getunconfirmedbalance", uriExtensionOpt = Some(walletExtension(walletName)))
  }
  ...
  private def getNewAddressInternal(
    ...
    bitcoindCall[BitcoinAddress]("getnewaddress", params, uriExtensionOpt = walletNameOpt.map(walletExtension)).map(
      addr => BitcoinAddress.fromScriptPubKey(addr.scriptPubKey, instance.network))
  }
  ...
  def getWalletInfo(walletName: String): Future[GetWalletInfoResult] = {
    bitcoindCall[GetWalletInfoResult]("getwalletinfo", uriExtensionOpt = Some(walletExtension(walletName)))
  }

  def keyPoolRefill(keyPoolSize: Int = 100, walletNameOpt: Option[String] = None): Future[Unit] = {
  ...
  def importPubKey(
      ...
      rescan: Boolean = true,
      walletNameOpt: Option[String] = None): Future[Unit] = {
    bitcoindCall[Unit]("importpubkey",
  ...
  def importPrivKey(
    ...
    bitcoindCall[Unit](
      "importprivkey",
      List(JsString(ECPrivateKeyUtil.toWIF(key, network)), JsString(account), JsBoolean(rescan)),
      uriExtensionOpt = walletNameOpt.map(walletExtension)
    )
  }
  ...
  def importMulti(
      ...
      rescan: Boolean = true,
      walletNameOpt: Option[String] = None): Future[Vector[ImportMultiResult]] = {
    bitcoindCall[Vector[ImportMultiResult]]("importmulti",
  ...
  def importPrunedFunds(
      ...
      walletNameOpt: Option[String] = None): Future[Unit] = {
    bitcoindCall[Unit]("importprunedfunds",
                       List(JsString(transaction.hex), JsString(txOutProof.hex)),
                       uriExtensionOpt = walletNameOpt.map(walletExtension))
  }

  def removePrunedFunds(txid: DoubleSha256DigestBE, walletNameOpt: Option[String]): Future[Unit] = {
  ...
  def removePrunedFunds(txid: DoubleSha256Digest, walletNameOpt: Option[String]): Future[Unit] = {
    removePrunedFunds(txid.flip, walletNameOpt)
  }

  def importWallet(filePath: String, walletNameOpt: Option[String] = None): Future[Unit] = {
    bitcoindCall[Unit]("importwallet", List(JsString(filePath)), uriExtensionOpt = walletNameOpt.map(walletExtension))
  }
    ...
    self.version.flatMap {
      case V21 | V20 | V19 | Experimental | Unknown =>
        bitcoindCall[SetWalletFlagResult]("setwalletflag",
                                          ...
                                          uriExtensionOpt = walletNameOpt.map(walletExtension))
      case V16 | V17 | V18 =>
        Future.failed(new UnsupportedOperationException("setwalletflag is not available for versions before 0.19"))
    }
  }
  ...
  def getBalances: Future[GetBalancesResult] = {
    self.version.flatMap {
      ...
      case V16 | V17 | V18 =>
        Future.failed(new UnsupportedOperationException("getbalances is not available for versions before 0.19"))
    }
  }
  ...
  def getBalances(walletName: String): Future[GetBalancesResult] = {
    self.version.flatMap {
      case V21 | V20 | V19 | Experimental | Unknown =>
        bitcoindCall[GetBalancesResult]("getbalances", uriExtensionOpt = Some(walletExtension(walletName)))
      case V16 | V17 | V18 =>
        Future.failed(new UnsupportedOperationException("getbalances is not available for versions before 0.19"))
    }
  }
  ...
  def setTxFee(feePerKB: Bitcoins, walletNameOpt: Option[String] = None): Future[Boolean] = {
  ...
  def walletLock(walletName: String): Future[Unit] = {
    bitcoindCall[Unit]("walletlock", uriExtensionOpt = Some(walletExtension(walletName)))
  }

  def walletPassphrase(passphrase: String, seconds: Int, walletNameOpt: Option[String] = None): Future[Unit] = {
  ...
  def walletPassphraseChange(
      ...
      walletNameOpt: Option[String] = None): Future[Unit] = {
    bitcoindCall[Unit]("walletpassphrasechange",
  ...
  def signRawTransactionWithWallet(
      transaction: Transaction,
      walletNameOpt: Option[String]): Future[SignRawTransactionWithWalletResult] = {
    bitcoindCall[SignRawTransactionWithWalletResult]("signrawtransactionwithwallet",
  ...
  def getAddressInfo(address: BitcoinAddress, walletNameOpt: Option[String] = None): Future[AddressInfoResult] = {
    self.version.flatMap {
      case V16 | V17 =>
      ...
      case V18 | V19 | V20 | Experimental | Unknown =>
        bitcoindCall[AddressInfoResultPostV18]("getaddressinfo",
                                               ...
                                               uriExtensionOpt = walletNameOpt.map(walletExtension))
      case V21 | Unknown =>
        bitcoindCall[AddressInfoResultPostV21]("getaddressinfo",
                                               List(JsString(address.value)),
                                               uriExtensionOpt = walletNameOpt.map(walletExtension))
    }
  }
  ...
  def sendMany(
    ...
    bitcoindCall[DoubleSha256DigestBE](
      "sendmany",
      List(JsString(""), jsonOutputs, JsNumber(minconf), JsString(comment), Json.toJson(subtractFeeFrom)),
      uriExtensionOpt = walletNameOpt.map(walletExtension)
    )
  }
  ...
  def walletCreateFundedPsbt(
    ...
    bitcoindCall[WalletCreateFundedPsbtResult](
      ...
      List(Json.toJson(inputs), jsonOutputs, JsNumber(locktime), Json.toJson(options), Json.toJson(bip32derivs)),
      uriExtensionOpt = walletNameOpt.map(walletExtension)
    )
  }
}
