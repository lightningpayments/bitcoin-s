...
object WalletStateDescriptorType extends StringFactory[WalletStateDescriptorType] {
  ...
  override def fromString(string: String): WalletStateDescriptorType = {
    fromStringOpt(string) match {
      ...
      case None =>
        sys.error(s"Could not find WalletStateDescriptorType for string=$string")
    }
  }
}
...
object WalletStateDescriptor extends StringFactory[WalletStateDescriptor] {
  ...
  val all: Vector[StringFactory[WalletStateDescriptor]] = Vector(SyncHeightDescriptor)
...
case class SyncHeightDescriptor(bestHash: DoubleSha256DigestBE, height: Int) extends WalletStateDescriptor {
...
object SyncHeightDescriptor extends WalletStateDescriptorFactory[SyncHeightDescriptor] {
