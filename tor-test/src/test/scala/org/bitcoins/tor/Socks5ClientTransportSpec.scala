...
class Socks5ClientTransportSpec extends BitcoinSAsyncTest with CachedTor {
  ...
  val settings = ConnectionPoolSettings(system).withConnectionSettings(clientConnectionSettings)

  it should "handle clear net addresses" in {
    assume(TorUtil.torEnabled, "TOR environment variable is not set")
    for {
      response <- Http().singleRequest(HttpRequest(uri = "http://blockstream.info/"), settings = settings)
    } yield {
  ...
  it should "handle onion addresses" in {
    ...
    for {
      response <- Http().singleRequest(
        HttpRequest(uri = "http://explorerzydxu5ecjrkwceayqybizmpjjznk5izmitf2modhcusuqlid.onion/"),
        settings = settings)
