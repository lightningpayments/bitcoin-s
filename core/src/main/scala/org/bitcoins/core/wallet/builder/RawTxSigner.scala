...
import org.bitcoins.core.wallet.utxo.{InputInfo, ScriptSignatureParams, UnassignedSegwitNativeInputInfo}
...
object RawTxSigner {
  ...
  val emptyInvariant: (Vector[ScriptSignatureParams[InputInfo]], Transaction) => Boolean = (_, _) => true

  def feeInvariant(expectedFeeRate: FeeUnit): (Vector[ScriptSignatureParams[InputInfo]], Transaction) => Boolean =
    addFeeRateInvariant(expectedFeeRate, emptyInvariant)

  private def addFeeRateInvariant(
      expectedFeeRate: FeeUnit,
      userInvariants: (Vector[ScriptSignatureParams[InputInfo]], Transaction) => Boolean): (
  ...
  def sign(utx: Transaction, utxoInfos: Vector[ScriptSignatureParams[InputInfo]]): Transaction = {
    sign(utx, utxoInfos, emptyInvariant, dummySign = false)
  }

  def sign(txWithInfo: FinalizedTxWithSigningInfo, expectedFeeRate: FeeUnit): Transaction = {
  ...
  def sign(
      ...
      utxoInfos: Vector[ScriptSignatureParams[InputInfo]],
      expectedFeeRate: FeeUnit,
      userInvariants: (Vector[ScriptSignatureParams[InputInfo]], Transaction) => Boolean): Transaction = {
  ...
  def sign(
    ...
    sign(txWithInfo.finalizedTx, txWithInfo.infos, invariants, dummySign = false)
  }

  def sign(
      utx: Transaction,
      utxoInfos: Vector[ScriptSignatureParams[InputInfo]],
      invariants: (Vector[ScriptSignatureParams[InputInfo]], Transaction) => Boolean,
    ...
    require(utxoInfos.distinct.length == utxoInfos.length, "All UTXOSatisfyingInfos must be unique. ")
    require(utxoInfos.forall(utxo => utx.inputs.exists(_.previousOutput == utxo.outPoint)),
            "All UTXOSatisfyingInfos must correspond to an input.")

    val signedTx =
      if (utxoInfos.exists(_.inputInfo.isInstanceOf[UnassignedSegwitNativeInputInfo])) {
        ...
        txWitness match {
          ...
          case _: TransactionWitness =>
            WitnessTransaction(btx.version, btx.inputs, btx.outputs, btx.lockTime, txWitness)
        }
      }
