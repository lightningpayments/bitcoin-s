...
object InitializeKeyManagerError {
  ...
  final case object BadEntropy extends Error("Bad Entropy") with KeyManagerInitializeError

  final case class EncryptionError(underlying: Throwable) extends Error(underlying) with KeyManagerInitializeError
  ...
  /** Wallet data was found in the
    ...
    */
  final case object WalletAlreadyExists extends Error("Wallet already exists") with KeyManagerInitializeError
