...
sealed abstract class SignerUtils {
  ...
  def doSign(
    ...
    if (isDummySignature) {
      ...
      TransactionSignatureCreator.createSig(unsignedTx, signingInfo, sign, hashType)
    }
  }
  ...
  def signSingle(
    ...
    val tx = spendingInfo.inputInfo match {
      case _: SegwitV0NativeInputInfo | _: P2SHNestedSegwitV0InputInfo | _: UnassignedSegwitNativeInputInfo =>
  ...
  protected def relevantInfo(
    ...
    (spendingInfo.signers, spendingInfo.output, inputIndex(spendingInfo, unsignedTx), spendingInfo.hashType)
  }

  protected def inputIndex(spendingInfo: InputSigningInfo[InputInfo], tx: Transaction): UInt32 = {
    tx.inputs.zipWithIndex
      .find(_._1.previousOutput == spendingInfo.outPoint) match {
      case Some((_, index)) => UInt32(index)
      case None =>
        throw new IllegalArgumentException("Transaction did not contain expected input.")
    }
  }
}
...
sealed abstract class Signer[-InputType <: InputInfo] extends SignerUtils {
  ...
  protected def updateScriptSigInSigComponent(
    ...
    val signedInput = TransactionInput(unsignedInput.previousOutput, scriptSignature, unsignedInput.sequence)
...
object BitcoinSigner extends SignerUtils {
  ...
  def sign(
      ...
      unsignedTx: Transaction,
      isDummySignature: Boolean,
      spendingInfoToSatisfy: ScriptSignatureParams[InputInfo]): TxSigComponent = {
    def spendingFrom[Info <: InputInfo](inputInfo: Info): ScriptSignatureParams[Info] = {
      spendingInfoToSatisfy.copy(inputInfo = inputInfo)
    }

    spendingInfoToSatisfy.inputInfo match {
      case empty: EmptyInputInfo =>
        EmptySigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(empty))
      case p2pk: P2PKInputInfo =>
        P2PKSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(p2pk))
      case p2pkh: P2PKHInputInfo =>
        P2PKHSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(p2pkh))
      case p2pKWithTimeout: P2PKWithTimeoutInputInfo =>
        P2PKWithTimeoutSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(p2pKWithTimeout))
      case p2sh: P2SHInputInfo =>
        P2SHSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(p2sh))
      case multiSig: MultiSignatureInputInfo =>
        MultiSigSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(multiSig))
      case lockTime: LockTimeInputInfo =>
        LockTimeSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(lockTime))
      case conditional: ConditionalInputInfo =>
        ConditionalSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(conditional))
      case p2wpkh: P2WPKHV0InputInfo =>
        P2WPKHSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(p2wpkh))
      case pw2sh: P2WSHV0InputInfo =>
        P2WSHSigner.sign(spendingInfo, unsignedTx, isDummySignature, spendingFrom(pw2sh))
  ...
  def sign(
    ...
    if (
      ...
      throw new IllegalArgumentException("Input has already been signed with this key")
    }
        ...
        tx match {
          case btx: NonWitnessTransaction =>
            val witnesses = psbt.inputMaps.map { map =>
              map.witnessScriptOpt.map(scriptWit => P2WSHWitnessV0(scriptWit.witnessScript))
            }
            val transactionWitness = TransactionWitness.fromWitOpt(witnesses)

            WitnessTransaction(btx.version, btx.inputs, btx.outputs, btx.lockTime, transactionWitness)
          case wtx: WitnessTransaction =>
...
sealed abstract class RawSingleKeyBitcoinSigner[-InputType <: RawInputInfo] extends Signer[InputType] {
  ...
  override def sign(
    ...
    val scriptSig =
      keyAndSigToScriptSig(partialSignature.pubKey.toPublicKey, partialSignature.signature, spendingInfoToSatisfy)

    updateScriptSigInSigComponent(unsignedTx, inputIndex.toInt, output, scriptSig)
  }
}
...
sealed abstract class EmptySigner extends Signer[EmptyInputInfo] {
  ...
  override def sign(
      ...
      unsignedTx: Transaction,
      isDummySignature: Boolean,
      spendingInfoToSatisfy: ScriptSignatureParams[EmptyInputInfo]): TxSigComponent = {
    val (_, output, inputIndex, _) = relevantInfo(spendingInfo, unsignedTx)

    val satisfyEmptyScriptSig = TrivialTrueScriptSignature

    updateScriptSigInSigComponent(unsignedTx, inputIndex.toInt, output, satisfyEmptyScriptSig)
  }
}
...
sealed abstract class MultiSigSigner extends Signer[MultiSignatureInputInfo] {
  ...
  override def sign(
      ...
      unsignedTx: Transaction,
      isDummySignature: Boolean,
      spendingInfoToSatisfy: ScriptSignatureParams[MultiSignatureInputInfo]): TxSigComponent = {
    val (_, output, inputIndex, _) =
      relevantInfo(spendingInfo, unsignedTx)
...
sealed abstract class P2SHSigner extends Signer[P2SHInputInfo] {
  ...
  override def sign(
      ...
      unsignedTx: Transaction,
      isDummySignature: Boolean,
      spendingInfoToSatisfy: ScriptSignatureParams[P2SHInputInfo]): TxSigComponent = {
    if (spendingInfoToSatisfy != spendingInfo) {
      ...
      val input =
        TransactionInput(spendingInfo.outPoint, EmptyScriptSignature, oldInput.sequence)

      val updatedTx =
        unsignedTx.updateInput(inputIndex.toInt, input)

      val nestedSpendingInfo = spendingInfoToSatisfy.copy(inputInfo = spendingInfoToSatisfy.inputInfo.nestedInputInfo)
      ...
      val p2sh =
        P2SHScriptSignature(i.scriptSignature, spendingInfoToSatisfy.inputInfo.redeemScript)
      ...
      val finalTx = signedTx match {
        case btx: NonWitnessTransaction =>
          BaseTransaction(version = btx.version, inputs = signedInputs, outputs = btx.outputs, lockTime = btx.lockTime)
        case wtx: WitnessTransaction =>
      ...
      P2SHTxSigComponent(transaction = finalTx, inputIndex = inputIndex, output = output, flags = flags)
    }
  }
}
...
sealed abstract class P2WPKHSigner extends Signer[P2WPKHV0InputInfo] {
  ...
  override def sign(
      ...
      unsignedTx: Transaction,
      isDummySignature: Boolean,
      spendingInfoToSatisfy: ScriptSignatureParams[P2WPKHV0InputInfo]): TxSigComponent = {
    if (spendingInfoToSatisfy != spendingInfo) {
      throw TxBuilderError.WrongSigner.exception
    } else {
      ...
      unsignedTx match {
        case wtx: WitnessTransaction =>
          ...
          val unsignedTxWitness = TransactionWitness(
            wtx.witness
              .updated(inputIndex.toInt, spendingInfoToSatisfy.inputInfo.scriptWitness)
              .toVector)
          ...
          val signature =
            doSign(unsignedTx, spendingInfo, signer.signLowR, hashType, isDummySignature)
...
sealed abstract class P2WSHSigner extends Signer[P2WSHV0InputInfo] {
  ...
  override def sign(
      ...
      unsignedTx: Transaction,
      isDummySignature: Boolean,
      spendingInfoToSatisfy: ScriptSignatureParams[P2WSHV0InputInfo]): TxSigComponent = {
    if (spendingInfoToSatisfy != spendingInfo) {
      throw TxBuilderError.WrongSigner.exception
    } else {
      val (_, output, inputIndex, _) = relevantInfo(spendingInfo, unsignedTx)
      ...
      val wtx = WitnessTransaction
        ...
        .updateWitness(inputIndex.toInt, spendingInfoToSatisfy.inputInfo.scriptWitness)

      val nestedSpendingInfo = spendingInfoToSatisfy.copy(inputInfo = spendingInfoToSatisfy.inputInfo.nestedInputInfo)

      val signedSigComponent = BitcoinSigner.sign(spendingInfo, wtx, isDummySignature, nestedSpendingInfo)

      val scriptWit =
        P2WSHWitnessV0(spendingInfoToSatisfy.inputInfo.scriptWitness.redeemScript, signedSigComponent.scriptSignature)
...
sealed abstract class LockTimeSigner extends Signer[LockTimeInputInfo] {
  ...
  override def sign(
      ...
      unsignedTx: Transaction,
      isDummySignature: Boolean,
      spendingInfoToSatisfy: ScriptSignatureParams[LockTimeInputInfo]): TxSigComponent = {
    val nestedSpendingInfo = spendingInfoToSatisfy.copy(inputInfo = spendingInfoToSatisfy.inputInfo.nestedInputInfo)

    BitcoinSigner.sign(spendingInfo, unsignedTx, isDummySignature, nestedSpendingInfo)
  }
}
...
sealed abstract class ConditionalSigner extends Signer[ConditionalInputInfo] {
  ...
  override def sign(
    ...
    val missingOpSigComponent = BitcoinSigner.sign(spendingInfo, unsignedTx, isDummySignature, nestedSpendingInfo)

    val scriptSig =
      ConditionalScriptSignature(missingOpSigComponent.scriptSignature, spendingInfoToSatisfy.inputInfo.condition)
