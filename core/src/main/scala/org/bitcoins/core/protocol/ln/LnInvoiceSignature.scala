...
object LnInvoiceSignature extends Factory[LnInvoiceSignature] {
  ...
  private case class LnInvoiceSignatureImpl(recoverId: UInt8, signature: ECDigitalSignature) extends LnInvoiceSignature

  def apply(recoverId: UInt8, signature: ECDigitalSignature): LnInvoiceSignature = {
