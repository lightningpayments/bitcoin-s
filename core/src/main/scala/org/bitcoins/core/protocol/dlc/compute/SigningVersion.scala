...
sealed abstract class SigningVersion {
  ...
  def calcOutcomeHash(descriptor: EventDescriptorTLV, bytes: ByteVector): ByteVector

  /** Calculates the bytes to sign for an event outcome */
  final def calcOutcomeHash(descriptor: EventDescriptorTLV, string: String): ByteVector = {
...
object SigningVersion extends StringFactory[SigningVersion] {
  ...
  final case object Mock extends SigningVersion {
    ...
    override def calcNonceTweak(nonce: SchnorrNonce, eventName: String): ByteVector = {
    ...
    override def calcOutcomeHash(descriptor: EventDescriptorTLV, byteVector: ByteVector): ByteVector =
  ...
  final case object BasicSHA256SigningVersion extends SigningVersion {
    ...
    override def calcOutcomeHash(descriptor: EventDescriptorTLV, byteVector: ByteVector): ByteVector = {
      descriptor match {
        case _: EnumEventDescriptorV0TLV | _: DigitDecompositionEventDescriptorV0TLV =>
