...
case class SetupDLC(fundingTx: Transaction, cets: Vector[(ECPublicKey, CETInfo)], refundTx: WitnessTransaction) {
  cets.foreach { case (msg, cetInfo) =>
    require(cetInfo.tx.inputs.size == 1,
  ...
  require(refundTx.inputs.size == 1,
  ...
  def getCETInfo(adaptorPoint: ECPublicKey): CETInfo = {
    cets.find(_._1 == adaptorPoint) match {
      ...
      case None =>
        throw new IllegalArgumentException(s"No CET found for the given adaptor point $adaptorPoint")
    }
  }
}
