...
import org.bitcoins.core.protocol.dlc.models.RoundingIntervals.{Interval, IntervalStart}
...
case class RoundingIntervals(intervalStarts: Vector[IntervalStart]) {
  if (intervalStarts.nonEmpty) {
    require(intervalStarts.init.zip(intervalStarts.tail).forall { case (i1, i2) =>
  ...
  def toTLV: RoundingIntervalsV0TLV = {
    RoundingIntervalsV0TLV(intervalStarts.map { case IntervalStart(outcome, amt) =>
  ...
  def intervalContaining(outcome: BigDecimal): Interval = {
    implicit val ord: Ordering[IntervalStart] =
      Ordering.by[IntervalStart, (BigDecimal, Long)](i => (i.firstOutcome, i.roundingMod))

    // Using Long.MaxValue guarantees that index will point to index of right endpoint of interval
    val index =
      NumberUtil.search(intervalStarts, IntervalStart(outcome, Long.MaxValue)) - 1
  ...
  def minRoundingWith(other: RoundingIntervals): RoundingIntervals = {
    ...
    def minMerge(
      ...
      if (thisIntervals.isEmpty) {
        val otherEnd = otherIntervals.map { case IntervalStart(startRange, otherMod) =>
          IntervalStart(startRange, Math.min(thisCurrentMod, otherMod))
        }
        builder.++=(otherEnd)
      } else if (otherIntervals.isEmpty) {
        val thisEnd = thisIntervals.map { case IntervalStart(startRange, thisMod) =>
        ...
        if (thisNextStart < otherNextStart) {
          ...
          minMerge(thisIntervals.tail, thisNextMod, otherIntervals, otherCurrentMod)
        } else if (thisNextStart > otherNextStart) {
          addInterval(otherNextStart, Math.min(otherNextMod, thisCurrentMod))
          minMerge(thisIntervals, thisCurrentMod, otherIntervals.tail, otherNextMod)
        } else {
          addInterval(thisNextStart, Math.min(thisNextMod, otherNextMod))
          minMerge(thisIntervals.tail, thisNextMod, otherIntervals.tail, otherNextMod)
        }
      }
    }
...
object RoundingIntervals {
  ...
  case class Interval(firstOutcome: BigDecimal, nextFirstOutcome: BigDecimal, roundingMod: Long) {
    require(firstOutcome < nextFirstOutcome, s"First outcome must come before last, $this")
  }
