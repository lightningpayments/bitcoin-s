...
class WalletCallbackTest extends BitcoinSWalletTest {
  ...
  it must "verify OnNewAddressGenerated callbacks are executed" in { fundedWallet: FundedWallet =>
  ...
  it must "verify OnTransactionProcessed callbacks are executed" in { fundedWallet: FundedWallet =>
  ...
  it must "verify OnTransactionBroadcast callbacks are executed" in { fundedWallet: FundedWallet =>
    val resultP: Promise[Transaction] = Promise()
  ...
  it must "verify OnReservedUtxos callbacks are executed when reserving" in { fundedWallet: FundedWallet =>
  ...
  it must "verify OnReservedUtxos callbacks are executed when un-reserving" in { fundedWallet: FundedWallet =>
    val resultP: Promise[Vector[SpendingInfoDb]] = Promise()
