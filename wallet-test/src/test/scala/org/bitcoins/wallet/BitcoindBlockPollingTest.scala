...
import org.bitcoins.testkit.wallet.{BitcoinSWalletTest, WalletAppConfigWithBitcoindNewestFixtures}
import org.bitcoins.testkitcore.util.TestUtil.bech32Address

import scala.concurrent.duration.DurationInt

class BitcoindBlockPollingTest extends WalletAppConfigWithBitcoindNewestFixtures {

  it must "properly setup and poll blocks from bitcoind" in { walletAppConfigWithBitcoind =>
    ...
    for {
      ...
      // Setup block polling
      _ <- BitcoindRpcBackendUtil.startBitcoindBlockPolling(wallet, bitcoind, 1.second)
      _ <- bitcoind.generateToAddress(6, bech32Address)

      // Wait for it to process
      _ <- AsyncUtil.awaitConditionF(() => wallet.getBalance().map(_ > Satoshis.zero), 1.second)
