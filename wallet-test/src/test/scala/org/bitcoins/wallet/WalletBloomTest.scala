...
class WalletBloomTest extends BitcoinSWalletTestCachedBitcoindNewest {
  ...
  it should "generate a bloom filter that matches the pubkeys in our wallet" in { param =>
  ...
  it should "generate a bloom filter that matches the outpoints in our wallet" in { param =>
    val WalletWithBitcoindRpc(walletApi, _) = param
    val wallet = walletApi.asInstanceOf[Wallet]
