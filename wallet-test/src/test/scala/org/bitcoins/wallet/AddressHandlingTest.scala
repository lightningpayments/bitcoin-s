...
class AddressHandlingTest extends BitcoinSWalletTest {
  ...
  it must "generate a new address for the default account and then find it" in { fundedWallet: FundedWallet =>
  ...
  it must "generate an address for a non default account and then find it" in { fundedWallet: FundedWallet =>
    val wallet = fundedWallet.wallet
  ...
  it must "generate the same unused address until it receives funds" in { fundedWallet: FundedWallet =>
    val wallet = fundedWallet.wallet
  ...
  it must "be safe to call getNewAddress multiple times in a row" in { fundedWallet: FundedWallet =>
    val wallet = fundedWallet.wallet
    ...
    for {
      ...
      assert(addresses.distinct.length == addresses.length, s"We receive an identical address!")

    }
  }

  it must "fail with an illegal state exception if the queue is full" in { fundedWallet: FundedWallet =>
    val wallet = fundedWallet.wallet
    ...
    //we failed
    val allCompletedF =
      AsyncUtil.retryUntilSatisfied(generatedF.forall(_.isCompleted), 250.millis)
  ...
  it must "get the correct spent addresses" in { fundedWallet: FundedWallet =>
    ...
    for {
      ...
      _ = assert(emptySpentAddresses.isEmpty,
  ...
  it must "get the correct funded addresses" in { fundedWallet: FundedWallet =>
    ...
    for {
      ...
      val diff = unspentDbs
        .map(_.output)
        .diff(fundedAddresses.map(tuple => TransactionOutput(tuple._2, tuple._1.scriptPubKey)))
  ...
  it must "correctly identify change addresses" in { fundedWallet: FundedWallet =>
    val wallet = fundedWallet.wallet
