...
import org.bitcoins.core.protocol.tlv.{EventDescriptorTLV, OracleEventTLV, OracleEventV0TLV}
...
case class EventDAO()(implicit val ec: ExecutionContext, override val appConfig: DLCOracleAppConfig)
  ...
  override protected def findByPrimaryKeys(ids: Vector[SchnorrNonce]): Query[EventTable, EventDb, Seq] =
    table.filter(_.nonce.inSet(ids))

  override protected def findAll(ts: Vector[EventDb]): Query[EventTable, EventDb, Seq] =
  ...
  def findByEventDescriptor(descriptorTLV: EventDescriptorTLV): Future[Vector[EventDb]] = {
    val query = table.filter(_.eventDescriptorTLV === descriptorTLV)

    safeDatabase.runVec(query.result.transactionally)
  }

  def findByOracleEventTLV(oracleEvent: OracleEventTLV): Future[Vector[EventDb]] = {
  ...
  class EventTable(tag: Tag) extends Table[EventDb](tag, schemaName, "events") {
    ...
    def fk: ForeignKeyQuery[_, RValueDb] = {
      foreignKey("fk_nonce", sourceColumns = nonce, targetTableQuery = rValueTable)(_.nonce)
    }

    def fkLabel: ForeignKeyQuery[_, RValueDb] = {
      foreignKey("fk_label", sourceColumns = eventName, targetTableQuery = rValueTable)(_.eventName)
    }
  }
}
