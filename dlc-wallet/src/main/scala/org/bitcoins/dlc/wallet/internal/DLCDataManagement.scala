...
private[bitcoins] trait DLCDataManagement { self: DLCWallet =>
  ...
  private[wallet] def getDLCAnnouncementDbs(dlcId: Sha256Digest): Future[
    (Vector[DLCAnnouncementDb], Vector[OracleAnnouncementDataDb], Vector[OracleNonceDb])] = {
  ...
  private[wallet] def getUsedOracleAnnouncements(
      ...
      nonceDbs: Vector[OracleNonceDb]): Vector[(OracleAnnouncementV0TLV, Long)] = {
        ...
        announcementData.find(_.id.contains(id)) match {
          case Some(data) =>
            ...
            if (used) {
              ...
              val eventTLV =
                OracleEventV0TLV(OrderedNonces(nonces), data.eventMaturity, data.eventDescriptor, data.eventId)
              Some((OracleAnnouncementV0TLV(data.announcementSignature, data.publicKey, eventTLV), data.id.get))
            } else None
          case None =>
      ...
      .map { case (id, nonceDbs) =>
        announcementData.find(_.id.contains(id)) match {
          case Some(data) =>
            ...
            val eventTLV =
              OracleEventV0TLV(OrderedNonces(nonces), data.eventMaturity, data.eventDescriptor, data.eventId)
            (OracleAnnouncementV0TLV(data.announcementSignature, data.publicKey, eventTLV), data.id.get)
          case None =>
            throw new RuntimeException(s"Error no data for announcement id $id")
        }
      }
  ...
  private[wallet] def getContractInfo(dlcId: Sha256Digest): Future[ContractInfo] = {
    for {
      contractData <- contractDataDAO.read(dlcId).map(_.get)
      (announcements, announcementData, nonceDbs) <- getDLCAnnouncementDbs(dlcId)
    } yield getContractInfo(contractData, announcements, announcementData, nonceDbs)
  }
      ...
      case enum: EnumContractDescriptor =>
        val oracleInfo =
          if (announcementTLVs.size == 1) {
            ...
            EnumMultiOracleInfo(contractDataDb.oracleThreshold, announcementTLVs)
          }
        ...
        val oracleInfo =
          if (announcementTLVs.size == 1) {
            ...
            contractDataDb.oracleParamsTLVOpt match {
              case Some(params) =>
                NumericMultiOracleInfo(contractDataDb.oracleThreshold, announcementTLVs, params)
              case None =>
                NumericExactMultiOracleInfo(contractDataDb.oracleThreshold, announcementTLVs)
            }
          }
        ContractInfo(contractDataDb.totalCollateral.satoshis, numeric, oracleInfo)
    }
  }

  private[wallet] def getDLCFundingData(contractId: ByteVector): Future[
    (DLCDb, DLCContractDataDb, DLCOfferDb, DLCAcceptDb, Vector[DLCFundingInputDb], ContractInfo)] = {
    for {
    ...
    } yield (dlcDb, contractData, dlcOffer, dlcAccept, fundingInputs, contractInfo)
  }

  private[wallet] def getDLCOfferData(
      dlcId: Sha256Digest): Future[(DLCDb, DLCContractDataDb, DLCOfferDb, Vector[DLCFundingInputDb], ContractInfo)] = {
    for {
  ...
  private[wallet] def getAllDLCData(contractId: ByteVector): Future[
    ...
    for {
      dlcDbOpt <- dlcDAO.findByContractId(contractId)
      dlcDb = dlcDbOpt.get
      (_, contractData, dlcOffer, dlcAccept, refundSig, contractInfo, fundingInputs, outcomeSigs) <-
        getAllDLCData(dlcDb.dlcId)
    } yield (dlcDb, contractData, dlcOffer, dlcAccept, refundSig, contractInfo, fundingInputs, outcomeSigs)
  }
  ...
  private[wallet] def getAllDLCData(dlcId: Sha256Digest): Future[
    ...
    } yield (dlcDb, contractData, dlcOffer, dlcAccept, refundSig.get, contractInfo, fundingInputs, outcomeSigs)
  }

  private[wallet] def fundingUtxosFromDb(
      dlcDb: DLCDb,
      fundingInputs: Vector[DLCFundingInputDb]): Future[Vector[ScriptSignatureParams[InputInfo]]] = {
    val outPoints =
    ...
    for {
      ...
      scriptSigParams <-
        FutureUtil.foldLeftAsync(Vector.empty[ScriptSignatureParams[InputInfo]], utxos) { (accum, utxo) =>
          transactionDAO
            .findByOutPoint(utxo.outPoint)
            .map(txOpt => utxo.toUTXOInfo(keyManager, txOpt.get.transaction) +: accum)
        }
    } yield scriptSigParams
  }

  private[wallet] def verifierFromAccept(accept: DLCAccept): Future[DLCSignatureVerifier] = {
    for {
      ...
      val builder =
        DLCTxBuilder(dlcOffer.toDLCOffer(contractInfo, offerFundingInputs, dlcDb, contractData), accept.withoutSigs)

      DLCSignatureVerifier(builder, dlcDb.isInitiator)
    }
  }

  private[wallet] def verifierFromDb(contractId: ByteVector): Future[DLCSignatureVerifier] = {
    getDLCFundingData(contractId).flatMap {
      case (dlcDb, contractData, dlcOffer, dlcAccept, fundingInputsDb, contractInfo) =>
        verifierFromDbData(dlcDb, contractData, dlcOffer, dlcAccept, fundingInputsDb, contractInfo)
    }
  }
  ...
  private[wallet] def builderFromDbData(
    ...
    val (localDbFundingInputs, remoteDbFundingInputs) = if (dlcDb.isInitiator) {
      (fundingInputsDb.filter(_.isInitiator), fundingInputsDb.filterNot(_.isInitiator))
    } else {
      (fundingInputsDb.filterNot(_.isInitiator), fundingInputsDb.filter(_.isInitiator))
    }

    for {
      localPrevTxs <- transactionDAO.findByTxIdBEs(localDbFundingInputs.map(_.outPoint.txIdBE))
      remotePrevTxs <-
        remoteTxDAO.findByTxIdBEs(remoteDbFundingInputs.map(_.outPoint.txIdBE))
    } yield {
      val localFundingInputs = matchPrevTxsWithInputs(inputs = localDbFundingInputs, prevTxs = localPrevTxs)

      val remoteFundingInputs = matchPrevTxsWithInputs(inputs = remoteDbFundingInputs, prevTxs = remotePrevTxs)
      ...
      val accept = dlcAccept.toDLCAcceptWithoutSigs(dlcDb.tempContractId, acceptFundingInputs)
  ...
  private[wallet] def verifierFromDbData(
    ...
    val builderF =
      builderFromDbData(dlcDb, contractData, dlcOffer, dlcAccept, fundingInputsDb, contractInfo)
  ...
  private[wallet] def signerFromDb(dlcId: Sha256Digest): Future[DLCTxSigner] = {
    for {
      (dlcDb, contractData, dlcOffer, dlcAccept, fundingInputsDb, contractInfo) <-
        getDLCFundingData(dlcId)
      signer <- signerFromDb(dlcDb, contractData, dlcOffer, dlcAccept, fundingInputsDb, contractInfo)
    } yield signer
  }
  ...
  private[wallet] def signerFromDb(
    ...
    for {
      ...
      val bip32Path = BIP32Path(
        dlcDb.account.path ++ Vector(BIP32Node(dlcDb.changeIndex.index, hardened = false),
  ...
  private[wallet] def executorFromDb(
    ...
    signerFromDb(dlcDb, contractDataDb, dlcOffer, dlcAccept, fundingInputsDb, contractInfo).map(DLCExecutor.apply)
  }

  private[wallet] def executorFromDb(dlcId: Sha256Digest): Future[DLCExecutor] = {
    signerFromDb(dlcId).map(DLCExecutor.apply)
  }

  private[wallet] def executorAndSetupFromDb(contractId: ByteVector): Future[(DLCExecutor, SetupDLC)] = {
    getAllDLCData(contractId).flatMap {
      case (dlcDb, contractData, dlcOffer, dlcAccept, refundSigs, contractInfo, fundingInputsDb, outcomeSigsDbs) =>
  ...
  private[wallet] def executorAndSetupFromDb(
      ...
      outcomeSigsDbs: Vector[DLCCETSignaturesDb]): Future[(DLCExecutor, SetupDLC)] = {

    executorFromDb(dlcDb, contractDataDb, dlcOffer, dlcAccept, fundingInputs, contractInfo)
      .flatMap { executor =>
