...
class DataMessageHandlerTest extends NodeUnitTest with CachedTor {
  ...
  it must "catch errors and not fail when processing an invalid payload" in { param: SpvNodeConnectedWithBitcoindV21 =>
    val SpvNodeConnectedWithBitcoindV21(spv, _) = param

    val sender = spv.peerMsgSenders(0)
    for {
      chainApi <- spv.chainApiFromDb()
      dataMessageHandler = DataMessageHandler(chainApi)(spv.executionContext, spv.nodeAppConfig, spv.chainConfig)
      ...
      _ <- recoverToSucceededIf[RuntimeException](chainApi.processHeaders(invalidPayload.headers))
  ...
  it must "verify OnMerkleBlock callbacks are executed" in { param: FixtureParam =>
    val SpvNodeConnectedWithBitcoindV21(spv, bitcoind) = param

    val resultP: Promise[(MerkleBlock, Vector[Transaction])] = Promise()

    val callback: OnMerkleBlockReceived = { (merkle: MerkleBlock, txs: Vector[Transaction]) =>
      Future {
    ...
    for {
      ...
      dataMessageHandler =
        DataMessageHandler(genesisChainApi)(spv.executionContext, spv.nodeAppConfig, spv.chainConfig)
  ...
  it must "verify OnBlockReceived callbacks are executed" in { param: FixtureParam =>
    val SpvNodeConnectedWithBitcoindV21(spv, bitcoind) = param
  ...
  it must "verify OnBlockHeadersReceived callbacks are executed" in { param: FixtureParam =>
    val SpvNodeConnectedWithBitcoindV21(spv, bitcoind) = param
  ...
  it must "verify OnCompactFilterReceived callbacks are executed" in { param: FixtureParam =>
    val SpvNodeConnectedWithBitcoindV21(spv, bitcoind) = param

    val resultP: Promise[Vector[(DoubleSha256Digest, GolombFilter)]] =
      Promise()
    val callback: OnCompactFiltersReceived = { (filters: Vector[(DoubleSha256Digest, GolombFilter)]) =>
      Future {
