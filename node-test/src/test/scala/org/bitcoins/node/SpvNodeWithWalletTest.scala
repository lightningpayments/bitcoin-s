...
class SpvNodeWithWalletTest extends NodeTestWithCachedBitcoindNewest {
  ...
  it must "load a bloom filter and receive information about received payments" in { param =>
    ...
    for {
      ...
      _ <- AsyncUtil.retryUntilSatisfiedF(() => walletContainsTx(wallet, ourTx.txid))
    } yield {
      succeed
    }
  }

  private def walletContainsTx(wallet: Wallet, txid: DoubleSha256DigestBE): Future[Boolean] = {
