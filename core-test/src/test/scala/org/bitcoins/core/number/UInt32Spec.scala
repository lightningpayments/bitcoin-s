...
class UInt32Spec extends Properties("UInt32") {
  ...
  property("subtractive identity") = Prop.forAll(NumberGenerator.uInt32s) { uInt32: UInt32 =>
  ...
  property("multiplying by zero gives us zero") = Prop.forAll(NumberGenerator.uInt32s) { uInt32: UInt32 =>
    uInt32 * UInt32.zero == UInt32.zero
  }

  property("multiplicative identity") = Prop.forAll(NumberGenerator.uInt32s) { uInt32: UInt32 =>
    uInt32 * UInt32.one == uInt32
  }

  property("multiply two UInt32s") = Prop.forAll(NumberGenerator.uInt32s, NumberGenerator.uInt32s) {
  ...
  property("< & >=") = Prop.forAll(NumberGenerator.uInt32s, NumberGenerator.uInt32s) { (num1: UInt32, num2: UInt32) =>
  ...
  property("<<") = Prop.forAllNoShrink(NumberGenerator.uInt32s, Gen.choose(0, 32)) { case (u32, shift) =>
    val r = Try(u32 << shift)
  ...
  property(">>") = Prop.forAll(NumberGenerator.uInt32s, Gen.choose(0, 100)) { case (u32, shift) =>
    val r = u32 >> shift
