...
class BIP32PathTest extends BitcoinSUnitTest {
  ...
  it must "derive public and private keys symmetrically" in {
    forAll(CryptoGenerators.extPrivateKey, HDGenerators.softBip32Path) { (priv, path) =>
  ...
  it must "fail to parse a path beginning with the wrong character" in {
    forAll(HDGenerators.bip32Path, Gen.alphaChar.suchThat(_ != 'm')) { (path, char) =>
      val badPathString = char.toString + path.toString.drop(1)
      assertThrows[IllegalArgumentException](BIP32Path.fromString(badPathString))
    }
  }
  ...
  it must "parse the paths from the BIP32 test vectors" in {
    val expected1 = BIP32Path(Vector(BIP32Node(0, hardened = true), BIP32Node(1, hardened = false)))
    assert(BIP32Path.fromString("m/0'/1") == expected1)

    val expected2 =
      BIP32Path(Vector(BIP32Node(0, hardened = true), BIP32Node(1, hardened = false), BIP32Node(2, hardened = true)))
  ...
  it must "parse the paths from the BIP32 test vector from bytes" in {
    ...
    val expected3 = BIP32Path(
      Vector(BIP32Node(0, hardened = true),
             ...
             BIP32Node(2, hardened = true),
             BIP32Node(2, hardened = false)))
    assert(BIP32Path.fromBytes(hex"0x80000000000000018000000200000002") == expected3)
  ...
  it must "do path diffing without altering the result" in {
    forAll(HDGenerators.diffableHDPaths, CryptoGenerators.extPrivateKey) { case ((short, long), xpriv) =>
