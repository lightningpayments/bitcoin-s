...
import org.bitcoins.crypto.{DoubleSha256Digest, DummyECDigitalSignature, ECPublicKey}
...
class TxSigComponentTest extends BitcoinSUnitTest {
  ...
  it should "correctly construct P2SHTxSigComponent" in {
    ...
    val btx = BaseTransaction(
      ...
      Vector(
        TransactionInput(
          ...
          P2SHScriptSignature(scriptSig = ConditionalScriptSignature(EmptyScriptSignature, condition = true),
    ...
    val nonWitnessP2SH =
      P2SHTxSigComponent(btx, UInt32.zero, TransactionOutput(Satoshis.one, p2shNoNest), Policy.standardFlags)
  ...
  it should "correctly construct WitnessTxSigComponentP2SH" in {
    ...
    val wtx = WitnessTransaction(
      ...
      Vector(TransactionOutput(Satoshis.one, EmptyScriptPubKey)),
      UInt32.zero,
      TransactionWitness(Vector(P2WPKHWitnessV0(pubKey, DummyECDigitalSignature)))
    )

    val witnessP2SH =
      P2SHTxSigComponent(wtx, UInt32.zero, TransactionOutput(Satoshis.one, p2shNested), Policy.standardFlags)
