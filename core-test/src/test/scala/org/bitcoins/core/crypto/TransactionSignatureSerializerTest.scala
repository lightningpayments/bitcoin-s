...
class TransactionSignatureSerializerTest extends BitcoinSUnitTest {
  ...
  "TransactionSignatureSerializer" must "correctly serialize an input that is being checked where another input in the same tx is using SIGHASH_ANYONECANPAY" in {
    ...
    val scriptPubKeyFromString =
      ScriptParser.fromString("0x21 0x035e7f0d4d0841bcd56c39337ed086b1a633ee770c1ffdd94ac552a95ac2ce0efc CHECKSIG")
    ...
    val serializedTxForSig: String =
      BytesUtil.encodeHex(TransactionSignatureSerializer.serializeForSignature(txSigComponent, HashType.sigHashAll))
  ...
  it must "correctly serialize a tx for signing with multiple inputs using SIGHASH_SINGLE" in {
    ...
    val spendingTx = Transaction(rawTx)
    val scriptPubKeyFromString =
      ScriptParser.fromString("DUP HASH160 0x14 0xdcf72c4fd02f5a987cf9b02f2fabfcac3341a87d EQUALVERIFY CHECKSIG")
    val scriptPubKey = ScriptPubKey.fromAsm(scriptPubKeyFromString)
    val txSigComponent =
      BaseTxSigComponent(spendingTx,
                         inputIndex,
  ...
  it must "serialize a p2wpkh with SIGHASH_SINGLE|SIGHASH_ANYONECANPAY" in {
    ...
    val txSigComponent =
      WitnessTxSigComponentRaw(wtx, inputIndex, TransactionOutput(amount, witScriptPubKey), Policy.standardFlags)

    val serializedForSig =
      TransactionSignatureSerializer.serializeForSignature(txSigComponent, HashType.sigHashSingleAnyoneCanPay)
  ...
  it must "work with the p2sh(p2wpkh) example in BIP143" in {
    ...
    val serialized =
      TransactionSignatureSerializer.serializeForSignature(txSigComponent = wtxSigComp, hashType = HashType.sigHashAll)

    BytesUtil.encodeHex(serialized) must be(expected)

    val hash =
      TransactionSignatureSerializer.hashForSignature(txSigComponent = wtxSigComp, hashType = HashType.sigHashAll)
  ...
  it must "serialize the BIP143 p2sh(p2wsh) examples" in {
    ...
    val serializationSigHashNone =
      TransactionSignatureSerializer.serializeForSignature(txSigComponent = wtxSigComp, hashType = HashType.sigHashNone)
    ...
    val serializationSigHashSingle =
      TransactionSignatureSerializer.serializeForSignature(txSigComponent = wtxSigComp,
  ...
  it must "make sure FindAndDelete is not applied to the BIP143 serialization algorithm" in {
    ...
    val serialized = TransactionSignatureSerializer.serializeForSignature(txSigComponent = wtxSigCompRaw,
        ...
        val correctScripts =
          creditingTxsInfo.flatMap { signInfo =>
            signInfo.signers.map { _ =>
              ...
              val oldBytes =
                TransactionSignatureSerializer.serializeForSignature(txSigComponent, signInfo.hashType)

              val newBytes =
                TransactionSignatureSerializer.serializeForSignature(spendingTx, signInfo, signInfo.hashType)
        ...
        val correctHashes =
          creditingTxsInfo.flatMap { signInfo =>
            signInfo.signers.map { _ =>
              ...
              val oldHash =
                TransactionSignatureSerializer.hashForSignature(txSigComponent, signInfo.hashType)

              val newHash =
                TransactionSignatureSerializer.hashForSignature(spendingTx, signInfo, signInfo.hashType)
        ...
        val correctScripts =
          creditingTxsInfo.flatMap { signInfo =>
            signInfo.signers.map { _ =>
              ...
              val oldScript =
                BitcoinScriptUtil.calculateScriptForSigning(txSigComponent, txSigComponent.output.scriptPubKey.asm)

              val newScript =
                BitcoinScriptUtil.calculateScriptForSigning(spendingTx, signInfo, signInfo.output.scriptPubKey.asm)
