...
class TransactionSignatureCreatorSpec extends Properties("TransactionSignatureCreatorSpec") {
  ...
  property("Must generate a valid signature for a p2pk transaction") =
    Prop.forAll(TransactionGenerators.signedP2PKTransaction) { case (txSignatureComponent: TxSigComponent, _) =>
  ...
  property("generate a valid signature for a p2sh transaction") =
    Prop.forAll(TransactionGenerators.signedP2SHTransaction) { case (txSignatureComponent: TxSigComponent, _) =>
      //run it through the interpreter
      val program = PreExecutionScriptProgram(txSignatureComponent)
      val result = ScriptInterpreter.run(program)
  ...
  property("generate a valid signature for a p2wpkh witness transaction") =
    Prop.forAllNoShrink(TransactionGenerators.signedP2WPKHTransaction) { case (wtxSigComponent, _) =>
  ...
  property("generate a valid signature for a p2wsh(old scriptPubkey tx) witness transaction") =
    Prop.forAllNoShrink(TransactionGenerators.signedP2WSHTransaction) { case (wtxSigComponent, _) =>
      val program = PreExecutionScriptProgram(wtxSigComponent)
      val result = ScriptInterpreter.run(program)
  ...
  property("generate a valid signature from a p2sh(p2wpkh) witness transaction") =
    Prop.forAllNoShrink(TransactionGenerators.signedP2SHP2WPKHTransaction) { case (wtxSigComponent, _) =>
      val program = PreExecutionScriptProgram(wtxSigComponent)
      val result = ScriptInterpreter.run(program)
      result == ScriptOk
    }
  ...
  property("generate a valid signature from a p2sh(p2wsh) witness tranasction") =
    Prop.forAllNoShrink(TransactionGenerators.signedP2SHP2WSHTransaction) { case (wtxSigComponent, _) =>
      val program = PreExecutionScriptProgram(wtxSigComponent)
      val result = ScriptInterpreter.run(program)
      Seq(ScriptErrorPushSize, ScriptOk).contains(result)
    }
