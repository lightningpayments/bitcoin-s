...
class TxoStateTest extends BitcoinSUnitTest {
  ...
  it must "read from string" in {
    ...
    TxoState.fromString("PendingConfirmationsReceived") must be(TxoState.PendingConfirmationsReceived)
    ...
    TxoState.fromString("PendingConfirmationsSpent") must be(TxoState.PendingConfirmationsSpent)
