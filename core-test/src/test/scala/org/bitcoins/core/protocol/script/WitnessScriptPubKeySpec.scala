...
class WitnessScriptPubKeySpec extends Properties("WitnessScriptPubKeySpec") {
  ...
  property("witnessScriptPubKeyV0 serialization symmetry") = Prop.forAll(ScriptGenerators.witnessScriptPubKeyV0) {
  ...
  property("witnessScriptPubKeyV0 fromAsm symmetry") = Prop.forAll(ScriptGenerators.witnessScriptPubKeyV0) {
    case (witScriptPubKeyV0, _) =>
      witScriptPubKeyV0 match {
        case p2wpkh: P2WPKHWitnessSPKV0 =>
