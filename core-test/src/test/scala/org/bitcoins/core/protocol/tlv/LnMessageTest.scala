...
class LnMessageTest extends BitcoinSUnitTest {
  ...
  "InitMessage" must "pass static test vectors" in {
    ...
    assert(Try(LnMessageFactory(InitTLV)(hex"00100000000001012a030104")).isSuccess)
    ...
    assert(Try(LnMessageFactory(InitTLV)(hex"001000000000010101010102")).isFailure)
  }
  ...
  "PongMessage" must "parse correctly" in {
    assert(LnMessage("001300020000") == LnMessage(PongTLV.forIgnored(ByteVector.fromValidHex("0000"))))
  }
