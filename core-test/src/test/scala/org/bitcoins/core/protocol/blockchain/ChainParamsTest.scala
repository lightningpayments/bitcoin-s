...
class ChainParamsTest extends BitcoinSUnitTest {
  ...
  it must "generate the correct txid for the genesis transaction" in {
    ...
    genesisTransaction.txId.hex must be(
      BytesUtil.flipEndianness("4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b"))
  }
  ...
  it must "generate the correct block hash for the testnet genesis block" in {
    TestNetChainParams.genesisBlock.blockHeader.hash.hex must be(
      BytesUtil.flipEndianness("000000000933ea01ad0ee984209779baaec3ced90fa3f408719526f8d77f4943"))
  }
  ...
  it must "generate the correct merkle root for the signet genesis block with different challenge spks" in {
    ...
    SigNetChainParams(default).genesisBlock.blockHeader.merkleRootHashBE.hex must be(
      "4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b")

    val p2pkh = P2PKHScriptPubKey(ECPublicKey.freshPublicKey)
    SigNetChainParams(p2pkh).genesisBlock.blockHeader.merkleRootHashBE.hex must be(
      "4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b")

    val trivialTrue = ScriptPubKey.fromAsm(Seq(OP_TRUE))
    SigNetChainParams(trivialTrue).genesisBlock.blockHeader.merkleRootHashBE.hex must be(
      "4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b")
  }
  ...
  it must "generate the correct blockheader hash for the genesis block on regtest" in {
    RegTestNetChainParams.genesisBlock.blockHeader.hash.hex must be(
      BytesUtil.flipEndianness("0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"))
  }
  ...
  it must "have the correct base58 prefix for MainNet" in {
    ...
    BytesUtil.encodeHex(MainNetChainParams.base58Prefixes(PubKeyAddress)) must be("00")
    BytesUtil.encodeHex(MainNetChainParams.base58Prefixes(ScriptAddress)) must be("05")
    BytesUtil.encodeHex(MainNetChainParams.base58Prefixes(SecretKey)) must be("80")
    BytesUtil.encodeHex(MainNetChainParams.base58Prefixes(ExtPublicKey)) must be("0488B21E".toLowerCase)
    BytesUtil.encodeHex(MainNetChainParams.base58Prefixes(ExtSecretKey)) must be("0488ADE4".toLowerCase)
  }

  it must "have the correct base58 prefix for TestNet" in {
    import Base58Type._
    BytesUtil.encodeHex(TestNetChainParams.base58Prefixes(PubKeyAddress)) must be("6f")
    BytesUtil.encodeHex(TestNetChainParams.base58Prefixes(ScriptAddress)) must be("c4")
    BytesUtil.encodeHex(TestNetChainParams.base58Prefixes(SecretKey)) must be("ef")
    BytesUtil.encodeHex(TestNetChainParams.base58Prefixes(ExtPublicKey)) must be("043587CF".toLowerCase)
    BytesUtil.encodeHex(TestNetChainParams.base58Prefixes(ExtSecretKey)) must be("04358394".toLowerCase)
  }

  it must "have the correct base58 prefix for RegTest" in {
    import Base58Type._
    BytesUtil.encodeHex(RegTestNetChainParams.base58Prefixes(PubKeyAddress)) must be("6f")
    BytesUtil.encodeHex(RegTestNetChainParams.base58Prefixes(ScriptAddress)) must be("c4")
    BytesUtil.encodeHex(RegTestNetChainParams.base58Prefixes(SecretKey)) must be("ef")
    BytesUtil.encodeHex(RegTestNetChainParams.base58Prefixes(ExtPublicKey)) must be("043587CF".toLowerCase)
    BytesUtil.encodeHex(RegTestNetChainParams.base58Prefixes(ExtSecretKey)) must be("04358394".toLowerCase)
  }

  it must "have the correct base58 prefix for SigNet" in {
    import Base58Type._
    BytesUtil.encodeHex(SigNetChainParams().base58Prefixes(PubKeyAddress)) must be("6f")
    BytesUtil.encodeHex(SigNetChainParams().base58Prefixes(ScriptAddress)) must be("c4")
    BytesUtil.encodeHex(SigNetChainParams().base58Prefixes(SecretKey)) must be("ef")
    BytesUtil.encodeHex(SigNetChainParams().base58Prefixes(ExtPublicKey)) must be("043587CF".toLowerCase)
    BytesUtil.encodeHex(SigNetChainParams().base58Prefixes(ExtSecretKey)) must be("04358394".toLowerCase)
  }
  ...
  it must "generate the correct default signet challenge" in {
    ...
    val pubKeys = Vector(
      ECPublicKey("03ad5e0edad18cb1f0fc0d28a3d4f1f3e445640337489abb10404f2d1e086be430"),
      ECPublicKey("0x0359ef5021964fe22d6f8e05b2463c9540ce96883fe3b278760f048f5189f2e6c4")
    )
  ...
  it must "compute the correct pow limits" in {
    val expectedTestMain = new BigInteger("26959946667150639794667015087019630673637144422540572481103610249215", 10)

    val expectedRegTest =
      new BigInteger("57896044618658097711785492504343953926634992332820282019728792003956564819967", 10)
