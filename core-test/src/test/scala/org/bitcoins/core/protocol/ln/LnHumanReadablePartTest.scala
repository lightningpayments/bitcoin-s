...
class LnHumanReadablePartTest extends BitcoinSUnitTest {
  ...
  it must "match the correct hrp with the correct network" in {
    ...
    LnHumanReadablePart(SigNet, mBtc) must be(LnHumanReadablePart(LnBitcoinSigNet, mBtcOpt))
  }
  ...
  it must "deserialize hrp from string" in {
    ...
    LnHumanReadablePart.fromString("lnbc") must be(LnHumanReadablePart(LnBitcoinMainNet))
    LnHumanReadablePart.fromString("lntb") must be(LnHumanReadablePart(LnBitcoinTestNet))
    LnHumanReadablePart.fromString("lnbcrt") must be(LnHumanReadablePart(LnBitcoinRegTest))
    LnHumanReadablePart.fromString("lntbs") must be(LnHumanReadablePart(LnBitcoinSigNet))

    LnHumanReadablePart.fromString("lnbc1m") must be(LnHumanReadablePart(LnBitcoinMainNet, mBtcOpt))
    LnHumanReadablePart.fromString("lntb1m") must be(LnHumanReadablePart(LnBitcoinTestNet, mBtcOpt))
    LnHumanReadablePart.fromString("lnbcrt1m") must be(LnHumanReadablePart(LnBitcoinRegTest, mBtcOpt))
    LnHumanReadablePart.fromString("lntbs1m") must be(LnHumanReadablePart(LnBitcoinSigNet, mBtcOpt))
  }
