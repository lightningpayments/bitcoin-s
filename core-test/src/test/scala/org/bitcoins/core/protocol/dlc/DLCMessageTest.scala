...
class DLCMessageTest extends BitcoinSJvmTest {
  ...
  it must "not allow a DLCTimeout where the contract times out before it matures" in {
    assertThrows[IllegalArgumentException](DLCTimeouts(BlockHeight(4), BlockHeight(2)))
    assertThrows[IllegalArgumentException](DLCTimeouts(BlockTime(UInt32(4)), BlockTime(UInt32(2))))
  }

  val dummyPubKey: ECPublicKey = ECPublicKey.freshPublicKey
  val dummyPubKey2: ECPublicKey = ECPublicKey.freshPublicKey

  val dummyAddress: BitcoinAddress = BitcoinAddress("1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa")

  val dummyStr: String =
    "00000000000000000008bba30d4d0fb53dcbffb601557de9f16d257d4f1985b7"

  val dummyOracle: EnumSingleOracleInfo = EnumSingleOracleInfo.dummyForKeys(ECPrivateKey.freshPrivateKey,
  ...
  it must "not allow a negative collateral for a DLCAccept" in {
    assertThrows[IllegalArgumentException](
      DLCAccept(
        ...
        changeSerialId = UInt64.one,
        CETSignatures(
          Vector(EnumOracleOutcome(Vector(dummyOracle), EnumOutcome(dummyStr)).sigPoint -> ECAdaptorSignature.dummy),
          dummySig),
  ...
  it must "be able to go back and forth between TLV and deserialized" in {
    forAll(TLVGen.dlcOfferTLVAcceptTLVSignTLV) { case (offerTLV, acceptTLV, signTLV) =>
  ...
  it must "be able to go back and forth between LN Message and deserialized" in {
    forAll(LnMessageGen.dlcOfferMessageAcceptMessageSignMessage) { case (offerMsg, acceptMsg, signMsg) =>
