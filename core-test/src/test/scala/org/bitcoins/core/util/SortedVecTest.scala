...
class SortedVecTest extends BitcoinSUnitTest {
  ...
  implicit val networkElementOrd: Ordering[NetworkElement] = { case (x: NetworkElement, y: NetworkElement) =>
  ...
  it should "sort correctly on construction with normal orderings" in {
    ...
    assert(SortedVec.sort(Vector(3.0, 2.0, 1.123)) == SortedVec(Vector(1.123, 2.0, 3.0)))
    assert(SortedVec.sort(Vector('c', 'b', 'a')) == SortedVec(Vector('a', 'b', 'c')))
    assert(SortedVec.sort(Vector("cab", "ceb", "abc")) == SortedVec(Vector("abc", "cab", "ceb")))
    assert(
      SortedVec.sort[SchnorrNonce, NetworkElement](Vector(
        SchnorrNonce("c4b89873c8753de3f0a9e94c4a6190badaa983513a6624a3469eb4577904bfea"),
  ...
  it should "fail to construct with unsorted vector under normal orderings" in {
    ...
    assertThrows[IllegalArgumentException](SortedVec(Vector("cab", "ceb", "abc")))
  ...
  it should "sort correctly on construction with custom orderings" in {
    ...
    implicit val remainderOrd: Ordering[Double] = { case (x: Double, y: Double) =>
      val diff = (x - x.floor) - (y - y.floor)
      if (diff > 0) 1 else if (diff == 0) 0 else -1
    }

    assert(SortedVec.sort(Vector(0.95, 123.123, 5.55)) == SortedVec(Vector(123.123, 5.55, 0.95)))
  }
  ...
  it should "fail to construct with unsorted vector under custom orderings" in {
    ...
    assertThrows[IllegalArgumentException](SortedVec(Vector(0.95, 5.55, 123.123)))
  }
  ...
  it should "correctly handle ordered list" in {
    ...
    assertThrows[IllegalArgumentException](SortedVec(wrongOrder)(SortedVec.forOrdered(nonces)))
  }
}
