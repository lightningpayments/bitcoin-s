...
class BinaryTreeTest extends BitcoinSUnitTest {
  ...
  it must "convert a binary tree to to a list with node values" in {
    ...
    val script =
      List(OP_IF, OP_IF, OP_1, OP_ELSE, OP_0, OP_ENDIF, OP_ELSE, OP_IF, OP_0, OP_ELSE, OP_1, OP_ENDIF, OP_ENDIF)
  ...
  it must "replace the left branch of a tree" in {
    ...
    tree.replace(tree.left.get, tree.right.get)() must be(
      Node[String]("Hello", Node("3", Empty, Leaf("4")), Node("3", Empty, Leaf("4"))))
  }

  it must "replace the right branch of a tree" in {
    val tree = Node[String]("Hello", Node("there", Leaf("1"), Leaf("2")), Node("3", Empty, Leaf("4")))
    tree.replace(tree.right.get, tree.left.get)() must be(
      Node[String]("Hello", Node("there", Leaf("1"), Leaf("2")), Node("there", Leaf("1"), Leaf("2"))))
  }
