...
class BytesToPushOntoStackFactoryTest extends BitcoinSUnitTest {
  ...
  "ScriptNumberFactory" must "represent the number 1" in {
    BytesToPushOntoStack.operations.contains(BytesToPushOntoStack(1)) must be(true)
    BytesToPushOntoStack.operations.contains(BytesToPushOntoStack(75)) must be(true)
  }
  ...
  it must "not allow creation of the script number -2" in {
    intercept[IllegalArgumentException] {
      BytesToPushOntoStack.operations.contains(BytesToPushOntoStack(-2)) must be(false)
    }
  }
