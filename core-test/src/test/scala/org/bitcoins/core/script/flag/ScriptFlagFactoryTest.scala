...
class ScriptFlagFactoryTest extends BitcoinSUnitTest {
  ...
  it must "find a DISCOURAGE_UPGRADABLE_NOPS flag" in {
    ScriptFlagFactory.fromString("DISCOURAGE_UPGRADABLE_NOPS") must be(ScriptVerifyDiscourageUpgradableNOPs)
  }
  ...
  it must "find a CHECKLOCKTIMEVERIFY flag" in {
    ScriptFlagFactory.fromString("CHECKLOCKTIMEVERIFY") must be(ScriptVerifyCheckLocktimeVerify)

  }

  it must "find a CHECKSEQUENCEVERIFY flag" in {
    ScriptFlagFactory.fromString("CHECKSEQUENCEVERIFY") must be(ScriptVerifyCheckSequenceVerify)
  }
