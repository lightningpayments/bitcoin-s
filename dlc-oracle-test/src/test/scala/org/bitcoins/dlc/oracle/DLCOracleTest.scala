...
class DLCOracleTest extends DLCOracleFixture {
  ...
  val testDescriptor: EnumEventDescriptorV0TLV = EnumEventDescriptorV0TLV(enumOutcomes)
  ...
  it must "calculate the correct staking address" in { dlcOracle: DLCOracle =>
    forAllAsync(ChainParamsGenerator.bitcoinNetworkParams) { network =>
      val expected =
        Bech32Address(P2WPKHWitnessSPKV0(dlcOracle.publicKey.publicKey), network)
  ...
  it must "correctly sign a message" in { dlcOracle: DLCOracle =>
    ...
    assert(dlcOracle.publicKey.verify(CryptoUtil.sha256(message).bytes, signature))
  }

  it must "get the correctly sorted nonces in an announcement " in { dlcOracle: DLCOracle =>
    val eventName = "test"
    val descriptorTLV =
    ...
    for {
      announcement <- dlcOracle.createNewAnnouncement(eventName = eventName, maturationTime = futureTime, descriptorTLV)
  ...
  it must "have same keys with different network configs" in { oracleA: DLCOracle =>
      ...
      for {
        ...
        (eventA, eventB) match {
          case (completedA: CompletedDigitDecompositionV0OracleEvent,
            ...
            assert(completedA.oracleAttestmentV0TLV == completedB.oracleAttestmentV0TLV)
          case (_, _) =>
  ...
  it must "create a new event and list it with pending" in { dlcOracle: DLCOracle =>
    val time = futureTime
  ...
  it must "create the same event twice and list them" in { dlcOracle: DLCOracle =>
    val time = futureTime
  ...
  it must "create two events and use incrementing key indexes" in { dlcOracle: DLCOracle =>
    val create1F =
  ...
  it must "fail to create an event with the same name" in { dlcOracle: DLCOracle =>
    for {
  ...
  it must "create an enum new event and get its details" in { dlcOracle: DLCOracle =>
    val time = futureTime
    val eventName = "test"
    ...
    for {
      ...
      val expectedAnnouncementTLV =
        OracleAnnouncementV0TLV(event.announcementSignature, event.pubkey, expectedEventTLV)
      ...
      assert(dlcOracle.publicKey.verify(announceBytes, event.announcementSignature))
    }
  }
    ...
    for {
      ...
      event match {
        case completedEvent: CompletedEnumV0OracleEvent =>
          ...
          assert(SchnorrDigitalSignature(completedEvent.nonces.head, completedEvent.attestation) == sig)
          assert(
    ...
    for {
      ...
      event match {
        case completedEvent: CompletedDigitDecompositionV0OracleEvent =>
          ...
          val digitOutcomes =
            Vector(DigitDecompositionAttestation(3), DigitDecompositionAttestation(2), DigitDecompositionAttestation(1))
          ...
          assert(SchnorrDigitalSignature(completedEvent.nonces.head, completedEvent.attestations.head) == signSig)

          // 100s Place signature Check
          val hash100 =
            SigningVersion.latest.calcOutcomeHash(descriptor, DigitDecompositionAttestation(3).bytes)
          val sig100 = completedEvent.signatures(1)
          assert(dlcOracle.publicKey.verify(hash100, sig100))
          assert(SchnorrDigitalSignature(completedEvent.nonces(1), completedEvent.attestations(1)) == sig100)

          // 10s Place signature Check
          val hash10 =
            SigningVersion.latest.calcOutcomeHash(descriptor, DigitDecompositionAttestation(2).bytes)
          val sig10 = completedEvent.signatures(2)
          assert(dlcOracle.publicKey.verify(hash10, sig10))
          assert(SchnorrDigitalSignature(completedEvent.nonces(2), completedEvent.attestations(2)) == sig10)

          // 1s Place signature Check
          val hash1 =
            SigningVersion.latest.calcOutcomeHash(descriptor, DigitDecompositionAttestation(1).bytes)
          val sig1 = completedEvent.signatures(3)
          assert(dlcOracle.publicKey.verify(hash1, sig1))
          assert(SchnorrDigitalSignature(completedEvent.nonces(3), completedEvent.attestations(3)) == sig1)
        case _: PendingOracleEvent | _: CompletedOracleEvent =>
          fail()
      }
    }
  ...
  it must "create and sign a non-base 10 large range event" in { dlcOracle: DLCOracle =>
    val outcome = -1931L
    ...
    for {
      ...
      event match {
        case completedEvent: CompletedDigitDecompositionV0OracleEvent =>
          ...
          // 100s Place signature Check
          val hash100 =
            SigningVersion.latest.calcOutcomeHash(descriptor, DigitDecompositionAttestation(7).bytes)
          val sig100 = completedEvent.signatures(1)
          assert(dlcOracle.publicKey.verify(hash100, sig100))
          assert(SchnorrDigitalSignature(completedEvent.nonces(1), completedEvent.attestations(1)) == sig100)
          ...
          // 10s Place signature Check
          val hash10 =
            SigningVersion.latest.calcOutcomeHash(descriptor, DigitDecompositionAttestation(8).bytes)
          val sig10 = completedEvent.signatures(2)
          assert(dlcOracle.publicKey.verify(hash10, sig10))
          assert(SchnorrDigitalSignature(completedEvent.nonces(2), completedEvent.attestations(2)) == sig10)
          ...
          // 1s Place signature Check
          val hash1 =
            SigningVersion.latest.calcOutcomeHash(descriptor, DigitDecompositionAttestation(11).bytes)
          val sig1 = completedEvent.signatures(3)
          assert(dlcOracle.publicKey.verify(hash1, sig1))
          assert(SchnorrDigitalSignature(completedEvent.nonces(3), completedEvent.attestations(3)) == sig1)
        case _: PendingOracleEvent | _: CompletedOracleEvent =>
  ...
  it must "create and sign a large range event with digits of 0" in { dlcOracle: DLCOracle =>
    val outcome = 2
    ...
    for {
      ...
      event match {
        case completedEvent: CompletedDigitDecompositionV0OracleEvent =>
          val digitOutcomes =
            Vector(DigitDecompositionAttestation(0), DigitDecompositionAttestation(1), DigitDecompositionAttestation(0))
          ...
          // 100s Place signature Check
          val hash100 =
            SigningVersion.latest.calcOutcomeHash(descriptor, DigitDecompositionAttestation(0).bytes)
          val sig100 = completedEvent.signatures.head
          assert(dlcOracle.publicKey.verify(hash100, sig100))
          assert(SchnorrDigitalSignature(completedEvent.nonces.head, completedEvent.attestations.head) == sig100)
          ...
          assert(dlcOracle.publicKey.verify(hash10, sig10))
          assert(SchnorrDigitalSignature(completedEvent.nonces(1), completedEvent.attestations(1)) == sig10)
          ...
          assert(dlcOracle.publicKey.verify(hash1, sig1))
          assert(SchnorrDigitalSignature(completedEvent.nonces(2), completedEvent.attestations(2)) == sig1)
        case _: PendingOracleEvent | _: CompletedOracleEvent =>
          fail()
      }
    }
  ...
  it must "create and sign a decomp event with a large num digits" in { dlcOracle: DLCOracle =>
  ...
  it must "fail to sign an event that doesn't exist" in { dlcOracle: DLCOracle =>
    val dummyNonce = SchnorrNonce(ECPublicKey.freshPublicKey.bytes.tail)
    recoverToSucceededIf[RuntimeException](dlcOracle.createAttestation(dummyNonce, EnumAttestation("testOutcomes")))
  }

  it must "fail to sign an enum outcome that doesn't exist" in { dlcOracle: DLCOracle =>
    recoverToSucceededIf[RuntimeException] {
      for {
        ...
        _ <- dlcOracle.createAttestation(nonce, EnumAttestation("not a real outcome"))
      } yield ()
    }
  }

  it must "sign a negative number for a unsigned digit decomp event that results in 0" in { dlcOracle: DLCOracle =>
    for {
      announcement <-
        dlcOracle.createNewDigitDecompAnnouncement(eventName = "test",
                                                   maturationTime = futureTime,
  ...
  it must "fail to sign an event with an outside nonce" in { dlcOracle: DLCOracle =>
  ...
  it must "fail to sign an event with a nonce not in the event db" in { dlcOracle: DLCOracle =>
    val ecKey = ECPublicKey.freshPublicKey
    val nonce = ecKey.schnorrNonce
  ...
  it must "fail to create an enum event with no outcomes" in { dlcOracle: DLCOracle =>
    assertThrows[IllegalArgumentException] {
      dlcOracle.createNewEnumAnnouncement("test", futureTime, Vector.empty)
    }
  }

  it must "fail to create an event with duplicate outcomes" in { dlcOracle: DLCOracle =>
  ...
  it must "create and sign a signed digit decomposition event" in { dlcOracle: DLCOracle =>
    val eventName = "signed"
    val maturationTime = futureTime
    val descriptor =
      SignedDigitDecompositionEventDescriptor(UInt16(2), UInt16(3), "unit", Int32(0))
    ...
    for {
      ...
      assert(OracleEvent.verifyAttestations(announcement, attestations, signingVersion = SigningVersion.latest))
    }
  }

  it must "create and sign a unsigned digit decomposition event" in { dlcOracle: DLCOracle =>
    val eventName = "unsigned"
    val maturationTime = futureTime
    val descriptor =
      UnsignedDigitDecompositionEventDescriptor(UInt16(2), UInt16(3), "unit", Int32(0))
  ...
  it must "fail to verify a unsigned digit decomposition event " in { dlcOracle: DLCOracle =>
    ...
    for {
      ...
      assert(!OracleEvent.verifyAttestations(announcement1, attestations2, signingVersion = SigningVersion.latest))
      assert(!OracleEvent.verifyAttestations(announcement2, attestations1, signingVersion = SigningVersion.latest))
    }
  }

  it must "create and delete signatures for an enum event" in { dlcOracle: DLCOracle =>
    val descriptor = TLVGen.enumEventDescriptorV0TLV.sampleSome
    val outcome = descriptor.outcomes.head
  ...
  it must "create and delete signatures for a decomp event" in { dlcOracle: DLCOracle =>
    val descriptor =
      UnsignedDigitDecompositionEventDescriptor(UInt16(2), UInt16(3), "unit", Int32(0))
  ...
  it must "fail to delete signatures for an unsigned enum event" in { dlcOracle: DLCOracle =>
    val descriptor =
      UnsignedDigitDecompositionEventDescriptor(UInt16(2), UInt16(3), "unit", Int32(0))
    ...
    for {
      ...
      signedEvent <- dlcOracle.findEvent("test").map(_.get)
      _ = assert(signedEvent.isInstanceOf[PendingDigitDecompositionV0OracleEvent])

      res <- recoverToSucceededIf[IllegalArgumentException](dlcOracle.deleteAttestation("test"))
    } yield res
  }

  it must "fail to delete signatures for an unsigned decomp event" in { dlcOracle: DLCOracle =>
    val descriptor = TLVGen.enumEventDescriptorV0TLV.sampleSome
  ...
  it must "delete enum announcements" in { dlcOracle =>
    val eventName = "test"
    val createdF =
      dlcOracle.createNewEnumAnnouncement(eventName, futureTime, Vector("0", "1", "2"))
    for {
      c <- createdF
  ...
  it must "fail to delete an announcement if there are attesations associated with it" in { dlcOracle =>
    val eventName = "test"
    val createdF =
      dlcOracle.createNewDigitDecompAnnouncement(eventName = eventName,
                                                 maturationTime = futureTime,
  ...
  it must "delete attestations, and then delete the announcement" in { dlcOracle: DLCOracle =>
    val eventName = "test"
    val createdF =
      dlcOracle.createNewDigitDecompAnnouncement(eventName = eventName,
                                                 maturationTime = futureTime,
