...
trait NodeCallbacks {
  ...
  def onCompactFiltersReceived: CallbackHandler[Vector[(DoubleSha256Digest, GolombFilter)], OnCompactFiltersReceived]
  ...
  def executeOnTxReceivedCallbacks(logger: Logger, tx: Transaction)(implicit ec: ExecutionContext): Future[Unit] = {
    onTxReceived.execute(tx,
                         (err: Throwable) => logger.error(s"${onTxReceived.name} Callback failed with error: ", err))
  }

  def executeOnBlockReceivedCallbacks(logger: Logger, block: Block)(implicit ec: ExecutionContext): Future[Unit] = {
    onBlockReceived.execute(
      block,
      (err: Throwable) => logger.error(s"${onBlockReceived.name} Callback failed with error: ", err))
  }

  def executeOnMerkleBlockReceivedCallbacks(logger: Logger, merkleBlock: MerkleBlock, txs: Vector[Transaction])(implicit
      ec: ExecutionContext): Future[Unit] = {
    onMerkleBlockReceived.execute(
      (merkleBlock, txs),
      (err: Throwable) => logger.error(s"${onMerkleBlockReceived.name} Callback failed with error: ", err))
  }

  def executeOnCompactFiltersReceivedCallbacks(
      logger: Logger,
      blockFilters: Vector[(DoubleSha256Digest, GolombFilter)])(implicit ec: ExecutionContext): Future[Unit] = {
    onCompactFiltersReceived.execute(
      blockFilters,
      (err: Throwable) => logger.error(s"${onCompactFiltersReceived.name} Callback failed with error: ", err))
  }

  def executeOnBlockHeadersReceivedCallbacks(logger: Logger, headers: Vector[BlockHeader])(implicit
      ec: ExecutionContext): Future[Unit] = {
    onBlockHeadersReceived.execute(
      headers,
      (err: Throwable) => logger.error(s"${onBlockHeadersReceived.name} Callback failed with error: ", err))
  }
}
...
object NodeCallbacks {
  ...
  private case class NodeCallbacksImpl(
      onCompactFiltersReceived: CallbackHandler[Vector[(DoubleSha256Digest, GolombFilter)], OnCompactFiltersReceived],
      onTxReceived: CallbackHandler[Transaction, OnTxReceived],
      onBlockReceived: CallbackHandler[Block, OnBlockReceived],
      onMerkleBlockReceived: CallbackHandler[(MerkleBlock, Vector[Transaction]), OnMerkleBlockReceived],
      onBlockHeadersReceived: CallbackHandler[Vector[BlockHeader], OnBlockHeadersReceived]
  ...
  val empty: NodeCallbacks =
    NodeCallbacks(Vector.empty, Vector.empty, Vector.empty, Vector.empty, Vector.empty)
  ...
  def apply(
    ...
    NodeCallbacksImpl(
      onCompactFiltersReceived =
        CallbackHandler[Vector[(DoubleSha256Digest, GolombFilter)], OnCompactFiltersReceived]("onCompactFilterReceived",
                                                                                              onCompactFiltersReceived),
      onTxReceived = CallbackHandler[Transaction, OnTxReceived]("onTxReceived", onTxReceived),
      onBlockReceived = CallbackHandler[Block, OnBlockReceived]("onBlockReceived", onBlockReceived),
      onMerkleBlockReceived =
        CallbackHandler[(MerkleBlock, Vector[Transaction]), OnMerkleBlockReceived]("onCompactFilterReceived",
                                                                                   onMerkleBlockReceived),
      onBlockHeadersReceived =
        CallbackHandler[Vector[BlockHeader], OnBlockHeadersReceived]("onCompactFilterReceived", onBlockHeadersReceived)
    )
  }
}
