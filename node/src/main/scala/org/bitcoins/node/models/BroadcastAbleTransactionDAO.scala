...
final case class BroadcastAbleTransactionDAO()(implicit override val appConfig: NodeAppConfig, val ec: ExecutionContext)
  ...
  override def createAll(ts: Vector[BroadcastAbleTransaction]): Future[Vector[BroadcastAbleTransaction]] =
    createAllNoAutoInc(ts, safeDatabase)

  /** Finds the rows that correlate to the given primary keys */
  override protected def findByPrimaryKeys(
      txIds: Vector[DoubleSha256DigestBE]): Query[BroadcastAbleTransactionTable, BroadcastAbleTransaction, Seq] = {
    table.filter(_.txid.inSet(txIds))
  }

  override protected def findAll(
      ts: Vector[BroadcastAbleTransaction]): Query[BroadcastAbleTransactionTable, BroadcastAbleTransaction, Seq] =
  ...
  class BroadcastAbleTransactionTable(tag: Tag)
      extends Table[BroadcastAbleTransaction](tag, schemaName, "broadcast_elements") {
    private type Tuple = (DoubleSha256DigestBE, ByteVector)

    private val fromTuple: Tuple => BroadcastAbleTransaction = { case (txid, bytes) =>
