...
class TestRpcUtilTest extends BitcoindFixturesCachedPairV21 {
  ...
  it should "be able to generate and sync blocks" in { nodes: NodePair[BitcoindV21RpcClient] =>
  ...
  it should "ble able to generate blocks with multiple clients and sync inbetween" in {
    nodes: NodePair[BitcoindV21RpcClient] =>
      ...
      for {
        ...
        _ <- BitcoindRpcTestUtil.generateAllAndSync(allClients, blocks = blocksToGenerate)
        ...
        assert(heightPostGeneration - heightPreGeneration == blocksToGenerate * allClients.length)
      }
  }

  it should "be able to find outputs of previous transactions" in { nodes: NodePair[BitcoindV21RpcClient] =>
    val NodePair(first, second) = nodes
    for {
      address <- second.getNewAddress
      txid <- first.sendToAddress(address, Bitcoins.one)
      hashes <- BitcoindRpcTestUtil.generateAndSync(Vector(first, second))
      vout <- BitcoindRpcTestUtil.findOutput(first, txid, Bitcoins.one, Some(hashes.head))
