...
import org.bitcoins.commons.jsonmodels.bitcoind.{Bip9SoftforkPostV19, GetBlockChainInfoResultPostV19}
...
class BitcoindV19RpcClientTest extends BitcoindFixturesFundedCachedV19 {
  ...
  it should "be able to start a V19 bitcoind instance" in { client: BitcoindV19RpcClient =>
    for {
      v <- client.version
    } yield {
  ...
  it should "get a block filter given a block hash" in { client: BitcoindV19RpcClient =>
    for {
  ...
  it should "be able to get blockchain info" in { client: BitcoindV19RpcClient =>
    for {
  ...
  it should "be able to set the wallet flag 'avoid_reuse'" in { client: BitcoindV19RpcClient =>
    for {
  ...
  it should "create a wallet with a passphrase" in { client: BitcoindV19RpcClient =>
    for {
  ...
  it should "check to see if the utxoUpdate input has been updated" in { client: BitcoindV19RpcClient =>
    val descriptor =
