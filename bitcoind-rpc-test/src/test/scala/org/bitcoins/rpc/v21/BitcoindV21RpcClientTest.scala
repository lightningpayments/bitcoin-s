...
class BitcoindV21RpcClientTest extends BitcoindFixturesFundedCachedV21 {
  ...
  it should "be able to start a V21 bitcoind instance" in { client: BitcoindV21RpcClient =>
    for {
      v <- client.version
    } yield assert(v == BitcoindVersion.V21)
  }

  it should "be able to get network info" in { freshClient: BitcoindV21RpcClient =>
    for {
  ...
  it should "get index info" in { client: BitcoindV21RpcClient =>
    def indexSynced(client: BitcoindRpcClient): Future[Boolean] = {
      client.getIndexInfo.map { indexes =>
        indexes("txindex").best_block_height == 101 && indexes("basic block filter index").best_block_height == 101
      }
    }
    for {
  ...
  it should "be able to get the address info for a given address" in { client: BitcoindV21RpcClient =>
    for {
  ...
  it should "get a block filter given a block hash" in { client: BitcoindV21RpcClient =>
    for {
  ...
  it should "be able to get the balances" in { client: BitcoindV21RpcClient =>
    for {
      ...
      assert(immatureBalance.mine.trusted.toBigDecimal + blockReward == newImmatureBalance.mine.trusted.toBigDecimal)
    }
  }

  it should "be able to get blockchain info" in { client: BitcoindV21RpcClient =>
    for {
      ...
      assert(preV19Info.softforks.values.exists(_.isInstanceOf[Bip9SoftforkPostV19]))
      assert(preV19Info.bestblockhash == bestHash)
    }
  }

  it should "be able to set the wallet flag 'avoid_reuse'" in { client: BitcoindV21RpcClient =>
    for {
  ...
  it should "create a wallet with a passphrase" in { client: BitcoindV21RpcClient =>
    for {
  ...
  it should "check to see if the utxoUpdate input has been updated" in { client: BitcoindV21RpcClient =>
    val descriptor =
      "pk(0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798)"

    val psbt =
      PSBT.fromBase64("cHNidP8BACoCAAAAAAFAQg8AAAAAABepFG6Rty1Vk+fUOR4v9E6R6YXDFkHwhwAAAAAAAA==")
  ...
  it should "correct create multisig and get its descriptor" in { client: BitcoindV21RpcClient =>
    ...
    for {
      ...
      assert(multiSigResult.descriptor.startsWith(s"sh(multi(2,${pubKey1.hex},${pubKey2.hex}))#"))
    }
  }
  ...
  it should "correct generate to a descriptor" in { client: BitcoindV21RpcClient =>
