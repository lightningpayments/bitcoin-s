...
import org.bitcoins.commons.jsonmodels.bitcoind.{FinalizedPsbt, NonFinalizedPsbt}
...
class PsbtRpcTest extends BitcoindFixturesCachedPairV17 {
  ...
  it should "decode all the BIP174 example PSBTs" in { nodePair: NodePair[BitcoindV17RpcClient] =>
  ...
  it should "convert raw TXs to PSBTs, process them  and then decode them" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    for {
      ...
      assert(decoded.inputs.exists(inputs => inputs.nonWitnessUtxo.isDefined || inputs.witnessUtxo.isDefined))
    }
  ...
  it should "finalize a simple PSBT" in { nodePair: FixtureParam =>
    ...
    for {
      ...
      txid <- BitcoindRpcTestUtil.fundBlockChainTransaction(client, otherClient, addr, Bitcoins.one)
      vout <- BitcoindRpcTestUtil.findOutput(client, txid, Bitcoins.one)
      newAddr <- client.getNewAddress
      psbt <-
        client.createPsbt(Vector(TransactionInput.fromTxidAndVout(txid, vout)), Map(newAddr -> Bitcoins(0.5)))
  ...
  it should "create a PSBT and then decode it" in { nodePair: FixtureParam =>
    ...
    for {
      ...
      psbt <- {
        ...
        val ourInput = TransactionInput(outpoint, ScriptSignature.empty, TransactionConstants.sequence)
        client.createPsbt(Vector(ourInput), Map(address -> Bitcoins(input.amount.toBigDecimal / 2)))
      }
  ...
  it should "create a funded wallet PSBT and then decode it" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    for {
      address <- client.getNewAddress
      input <- client.listUnspent.map(_.filter(_.spendable).head)
      psbt <- {
        val outpoint =
          TransactionOutPoint(input.txid.flip, UInt32(input.vout))
        val ourInput = TransactionInput(outpoint, ScriptSignature.empty, TransactionConstants.sequence)
        client.walletCreateFundedPsbt(Vector(ourInput), Map(address -> Bitcoins(input.amount.toBigDecimal / 2)))
      }
