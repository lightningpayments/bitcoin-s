...
import org.bitcoins.commons.jsonmodels.bitcoind.RpcOpts.{AddressType, LabelPurpose, SignRawTransactionOutputParameter}
...
class BitcoindV17RpcClientTest extends BitcoindFixturesCachedPairV17 {
  ...
  it must "have our BitcoindRpcClient work with .hashCode() and equals" in { nodePair =>
  ...
  it should "sign a raw transaction with wallet keys" in { nodePair: FixtureParam =>
    val NodePair(client, otherClient) = nodePair
    for {
      rawTx <-
        BitcoindRpcTestUtil.createRawCoinbaseTransaction(client, otherClient)
  ...
  it should "sign a raw transaction with private keys" in { nodePair: FixtureParam =>
    ...
    val scriptPubKeys =
      List("76a91460baa0f494b38ce3c940dea67f3804dc52d1fb9488ac", "76a914669b857c03a5ed269d5d85a1ffac9ed5d663072788ac")
    ...
    for {
      ...
      signed <-
        client.signRawTransactionWithKey(rawTx, privkeys.toVector.map(_.toPrivateKey), utxoDeps)
    } yield assert(signed.complete)
  }

  it should "be able to get the address info for a given address" in { nodePair: FixtureParam =>
    val NodePair(client, _) = nodePair
    for {
  ...
  it should "be able to get the address info for a given P2SHSegwit address" in { nodePair: FixtureParam =>
    val NodePair(client, _) = nodePair
    for {
  ...
  it should "be able to get the address info for a given Legacy address" in { nodePair: FixtureParam =>
    val NodePair(client, _) = nodePair
    for {
  ...
  it should "be able to get the address info for a given Bech32 address" in { nodePair: FixtureParam =>
    val NodePair(client, _) = nodePair
    for {
  ...
  it should "be able to get the amount received by a label" in { nodePair: FixtureParam =>
    val NodePair(client, otherClient) = nodePair
    for {
  ...
  it should "set labels" in { nodePair: FixtureParam =>
    ...
    for {
      addr <- client.getNewAddress
      _ <- BitcoindRpcTestUtil.fundBlockChainTransaction(otherClient, client, addr, btc)

      newestBlock <- otherClient.getBestBlockHash
      _ <- AsyncUtil.retryUntilSatisfiedF(() => BitcoindRpcTestUtil.hasSeenBlock(client, newestBlock))
  ...
  it should "list amounts received by all labels" in { nodePair: FixtureParam =>
    ...
    for {
      ...
      _ <- AsyncUtil.retryUntilSatisfiedF(() => BitcoindRpcTestUtil.hasSeenBlock(client, newBlock))
