...
class MempoolRpcTest extends BitcoindFixturesCachedPairV21 {
  ...
  it should "be able to find a transaction sent to the mem pool" in { nodePair: FixtureParam =>
  ...
  it should "be able to find a verbose transaction in the mem pool" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    val otherClient = nodePair.node2
    for {
      transaction <-
  ...
  it must "fail to find a mempool entry and return None" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    val txid = DoubleSha256Digest.empty
    val resultF = for {
  ...
  it should "be able to prioritise a mem pool transaction" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    val otherClient = nodePair.node2
    for {
  ...
  it should "be able to find mem pool ancestors and descendants" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    for {
      _ <- client.getNewAddress.flatMap(client.generateToAddress(1, _))
      address1 <- client.getNewAddress
      txid1 <- BitcoindRpcTestUtil.fundMemPoolTransaction(client, address1, Bitcoins(2))
      mempool <- client.getRawMemPool
      address2 <- client.getNewAddress

      createdTx <- {
        val input: TransactionInput =
          TransactionInput(TransactionOutPoint(txid1.flip, UInt32.zero), ScriptSignature.empty, UInt32.max - UInt32.one)
        client
  ...
  it should "be able to save the mem pool to disk" in { nodePair: FixtureParam =>
    val client = nodePair.node1
