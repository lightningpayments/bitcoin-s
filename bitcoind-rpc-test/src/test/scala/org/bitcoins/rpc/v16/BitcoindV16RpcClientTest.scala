...
import org.bitcoins.testkit.rpc.{BitcoindFixturesCachedPairV16, BitcoindRpcTestUtil}
...
class BitcoindV16RpcClientTest extends BitcoindFixturesCachedPairV16 {
  ...
  it should "be able to sign a raw transaction" in { nodePair: FixtureParam =>
    ...
    for {
      ...
      _ <- AsyncUtil.retryUntilSatisfiedF(() => BitcoindRpcTestUtil.hasSeenBlock(client, recentBlock), 1.second)
      (utxoTxid, utxoVout) <-
        client.listUnspent
  ...
  it should "be able to sign a raw transaction with private keys" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    ...
    for {
      ...
      signed <- client.signRawTransaction(rawTx, utxoDeps, privkeys.toVector.map(_.toPrivateKey))
    } yield assert(signed.complete)
  }

  it should "be able to send from an account to an addresss" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    val otherClient = nodePair.node2
    for {
  ...
  it should "be able to get and set the account for a given address" in { nodePair: FixtureParam =>
    val client = nodePair.node1
  ...
  it should "be able to get all addresses belonging to an account" in { nodePair: FixtureParam =>
    val client = nodePair.node1
    for {
  ...
  it should "be able to move funds from one account to another" in { nodePair: FixtureParam =>
    val client = nodePair.node1
