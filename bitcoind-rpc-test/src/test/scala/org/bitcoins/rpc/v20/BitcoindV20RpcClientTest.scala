...
class BitcoindV20RpcClientTest extends BitcoindFixturesFundedCachedV20 {
  ...
  it should "be able to start a V20 bitcoind instance" in { client: BitcoindV20RpcClient =>
    for {
      v <- client.version
    } yield assert(v == BitcoindVersion.V20)
  }

  it should "get a block filter given a block hash" in { client: BitcoindV20RpcClient =>
    for {
  ...
  it should "be able to get blockchain info" in { client: BitcoindV20RpcClient =>
    for {
  ...
  it should "be able to set the wallet flag 'avoid_reuse'" in { client: BitcoindV20RpcClient =>
    for {
  ...
  it should "create a wallet with a passphrase" in { client: BitcoindV20RpcClient =>
    for {
  ...
  it should "check to see if the utxoUpdate input has been updated" in { client: BitcoindV20RpcClient =>
    val descriptor =
  ...
  it should "correct create multisig and get its descriptor" in { client: BitcoindV20RpcClient =>
  ...
  it should "correct generate to a descriptor" in { client: BitcoindV20RpcClient =>
