...
class BlockHeaderDAOTest extends ChainDbUnitTest {
  ...
  it should "insert and read the genesis block header back" in { blockHeaderDAO: BlockHeaderDAO =>
  ...
  it must "delete a block header in the database" in { blockHeaderDAO: BlockHeaderDAO =>
  ...
  it must "fail to find a header closest to a epoch second" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
    val createdF = blockHeaderDAO.create(blockHeader)
  ...
  it must "find the closest block to the given epoch second" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
    val createdF = blockHeaderDAO.create(blockHeader)

    val headerDbF = createdF.flatMap(_ => blockHeaderDAO.findClosestToTime(UInt32(TimeUtil.currentEpochSecond)))
  ...
  it must "find the block at given epoch second" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
    val createdF = blockHeaderDAO.create(blockHeader)

    val headerDbF = createdF.flatMap(_ => blockHeaderDAO.findClosestToTime(blockHeader.time))
  ...
  it must "find all the headers before to the given epoch second" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
    val createdF = blockHeaderDAO.create(blockHeader)

    val headerDbsF = createdF.flatMap(_ => blockHeaderDAO.findClosestBeforeTime(UInt32(TimeUtil.currentEpochSecond)))
  ...
  it must "retrieve the best chain tip saved in the database" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
  ...
  it must "retrieve a block header by height" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
  ...
  it must "find the height of the longest chain" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
    val createdF = blockHeaderDAO.create(blockHeader)
  ...
  it must "get an ancestor at a specified height" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
    val createdF = blockHeaderDAO.create(blockHeader)

    val genesisF = createdF.flatMap(created => blockHeaderDAO.getAncestorAtHeight(created, 0))
  ...
  it must "implement getNAncestors correctly" in { blockHeaderDAO: BlockHeaderDAO =>
    val blockHeader = BlockHeaderHelper.buildNextHeader(genesisHeaderDb)
    val createdF = blockHeaderDAO.create(blockHeader)

    val noGenesisAncestorsF =
      blockHeaderDAO.getNAncestors(childHash = genesisHeaderDb.blockHeader.hashBE, n = 1)

    val foundGenesisF =
      blockHeaderDAO.getNAncestors(childHash = genesisHeaderDb.blockHeader.hashBE, n = 0)
    ...
    val oneChildF = for {
      created <- createdF
      children <- blockHeaderDAO.getNAncestors(childHash = created.blockHeader.hashBE, n = 1)
    } yield {
  ...
  it must "get the correct chain tip" in { blockerHeaderDAO: BlockHeaderDAO =>
    val db1 =
      BlockHeaderDbHelper.fromBlockHeader(1, BigInt(1, hex"fffef2bf0566ab".toArray), ChainTestUtil.blockHeader562462)
  ...
  it must "successfully map a max chain work block" in { blockerHeaderDAO: BlockHeaderDAO =>
    val bytes = ByteVector.fill(32)(0xff)
    val chainWork = BigInt(1, bytes.toArray)

    val db =
      BlockHeaderDbHelper.fromBlockHeader(1, chainWork, ChainTestUtil.blockHeader562462)
  ...
  it must "get blockchains from the genesis header" in { blockHeaderDAO: BlockHeaderDAO =>
  ...
  it must "successfully getBlockchainsBetweenHeights" in { blockerHeaderDAO: BlockHeaderDAO =>
    ...
    val chain1 = Vector(
      BlockHeaderDbHelper.fromBlockHeader(3, BigInt(2), ChainTestUtil.blockHeader562464),
      BlockHeaderDbHelper.fromBlockHeader(2, BigInt(1), ChainTestUtil.blockHeader562463),
    ...
    val expectedChains =
      Vector(Blockchain(chain1), Blockchain(chain2), Blockchain(chain3), Blockchain(chain4))
