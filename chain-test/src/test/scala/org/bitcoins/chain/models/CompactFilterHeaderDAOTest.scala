...
class CompactFilterHeaderDAOTest extends ChainDbUnitTest {
  ...
  it must "create and read a filter header from the database" in { filterHeaderDAO =>
  ...
  it must "get the best filter header that has a block header associated with it" in { filterHeaderDAO =>
    val blockHeaderDAO = BlockHeaderDAO()
    val blockHeaderDb = {
      BlockHeaderHelper.buildNextHeader(ChainTestUtil.regTestGenesisHeaderDb)
    }
  ...
  it must "fail to find a filter header if the block header is not in the db" in { filterHeaderDAO =>
    val blockHeaderDb = {
      BlockHeaderHelper.buildNextHeader(ChainTestUtil.regTestGenesisHeaderDb)
    }
  ...
  it must "find the filter header with the heaviest work" in { filterHeaderDAO =>
    val blockHeaderDAO = BlockHeaderDAO()
  ...
  private def randomFilterHeader(blockHeader: BlockHeaderDb): CompactFilterHeaderDb = {
    CompactFilterHeaderDb(
      CryptoGenerators.doubleSha256Digest.sample.get.flip,
      filterHashBE = CryptoGenerators.doubleSha256Digest.sample.get.flip,
      previousFilterHeaderBE = CryptoGenerators.doubleSha256Digest.sample.get.flip,
