...
import org.bitcoins.core.api.chain.db.{BlockHeaderDb, BlockHeaderDbHelper, CompactFilterHeaderDb}
...
import org.bitcoins.testkit.chain.{BlockHeaderHelper, ChainDbUnitTest, ChainTestUtil, ChainUnitTest}
...
class ChainHandlerTest extends ChainDbUnitTest {
  ...
  it must "process a new valid block header, and then be able to fetch that header" in { chainHandler: ChainHandler =>
    val newValidHeader =
  ...
  it must "not fail ChainHandler.processHeaders() with empty headers collection" in { chainHandler: ChainHandler =>
    for {
      _ <- chainHandler.processHeaders(Vector.empty)
    } yield succeed
  }

  it must "fail if we give a header that cannot be connected to anything" in { chainHandler: ChainHandler =>
  ...
  it must "handle a very basic reorg where one chain is one block behind the best chain" in {
    chainHandler: ChainHandler =>
      ...
      val assertBBestHashF = for {
        ...
        checkReorgHeaders(header1 = newHeaderB, header2 = newHeaderC, bestHash = bestHash)
      }
  ...
  // G -> A -> B
  // G -> C -> D -> E
  it must "handle a reorg where one chain is two blocks behind the best chain" in { chainHandler: ChainHandler =>
    for {
  ...
  it must "NOT reorg to a shorter chain that just received a new block" in { chainHandler: ChainHandler =>
    val reorgFixtureF = buildChainHandlerCompetingHeaders(chainHandler)
    val chainHandlerCF = reorgFixtureF.map(_.chainApi)
    val newHeaderBF = reorgFixtureF.map(_.headerDb1)
    val newHeaderCF = reorgFixtureF.map(_.headerDb2)
    ...
    val chainHandlerEWithHeaderF: Future[(ChainApi, BlockHeaderDb)] = for {
      ...
      chainHandlerE <- chainHandler.processHeaders(Vector(headerD.blockHeader, headerE.blockHeader))
  ...
  it must "get the highest filter header" in { chainHandler: ChainHandler =>
    {
      for {
        ...
        assert(genesisFilterHeader.contains(ChainUnitTest.genesisFilterHeaderDb))
        assert(genesisFilterHeader.head.filterHeader == ChainUnitTest.genesisFilterHeaderDb.filterHeader)
        assert(count == 0)
      }
    }
  }

  it must "NOT create a filter header for an unknown block" in { chainHandler: ChainHandler =>
    {
      val firstFilterHeader = FilterHeader(filterHash =
                                             DoubleSha256Digest.fromBytes(ECPrivateKey.freshPrivateKey.bytes),
                                           prevHeaderHash = DoubleSha256Digest.empty)
      val newChainHandlerF =
        chainHandler.processFilterHeader(firstFilterHeader,
  ...
  it must "verify a batch of filter headers" in { chainHandler: ChainHandler =>
    val goodBatch = Vector(
      ...
      CompactFilterHeaderDb(
        hashBE = DoubleSha256DigestBE.fromHex("000102030405060708090a0b0c0d0e0f000102030405060708090a0b0c0d0e0f"),
        previousFilterHeaderBE = ChainUnitTest.genesisFilterHeaderDb.hashBE,
        height = 1,
        filterHashBE = DoubleSha256DigestBE.fromHex("555152535455565758595a5b5c5d5e5f555152535455565758595a5b5c5d5e5f"),
    ...
    val selfReferenceFilterHeaderBatch = Vector(
      ...
      CompactFilterHeaderDb(
        ...
        previousFilterHeaderBE =
          DoubleSha256DigestBE.fromHex("000102030405060708090a0b0c0d0e0f000102030405060708090a0b0c0d0e0f"),
        height = 1,
        filterHashBE = DoubleSha256DigestBE.fromHex("555152535455565758595a5b5c5d5e5f555152535455565758595a5b5c5d5e5f"),
        blockHashBE = DoubleSha256DigestBE.fromHex("aaa1a2a3a4a5a6a7a8a9aaabacadaeafaaa1a2a3a4a5a6a7a8a9aaabacadaeaf")
      )
    ...
    val unknownFilterHeaderBatch = Vector(
      ...
      CompactFilterHeaderDb(
        hashBE = DoubleSha256DigestBE.fromHex("000102030405060708090a0b0c0d0e0f000102030405060708090a0b0c0d0e0f"),
        previousFilterHeaderBE =
          DoubleSha256DigestBE.fromHex("555152535455565758595a5b5c5d5e5f555152535455565758595a5b5c5d5e5f"),
        height = 1,
        filterHashBE = DoubleSha256DigestBE.fromHex("555152535455565758595a5b5c5d5e5f555152535455565758595a5b5c5d5e5f"),
        blockHashBE = DoubleSha256DigestBE.fromHex("aaa1a2a3a4a5a6a7a8a9aaabacadaeafaaa1a2a3a4a5a6a7a8a9aaabacadaeaf")
      )
    ...
    for {
      ...
      _ <- recoverToSucceededIf[IllegalArgumentException](chainHandler.verifyFilterHeaders(invalidFilterHeaderBatch))
      _ <- recoverToSucceededIf[IllegalArgumentException](
        chainHandler.verifyFilterHeaders(selfReferenceFilterHeaderBatch))
      _ <- recoverToSucceededIf[IllegalArgumentException](chainHandler.verifyFilterHeaders(unknownFilterHeaderBatch))
    } yield succeed
  }
  ...
  it must "get the highest filter" in { chainHandler: ChainHandler =>
    {
      for {
        ...
        assert(genesisFilter.head.golombFilter == ChainUnitTest.genesisFilterDb.golombFilter)
      }
    }
  }
  ...
  it must "NOT create an unknown filter" in { chainHandler: ChainHandler =>
    {
      val unknownHashF = for {
        ...
        firstFilter = CompactFilterMessage(blockHash = blockHashBE.flip, filter = golombFilter)
        firstFilterHeader = FilterHeader(filterHash = DoubleSha256Digest.fromBytes(ECPrivateKey.freshPrivateKey.bytes),
  ...
  it must "NOT create a filter of an unknown block" in { chainHandler: ChainHandler =>
    {
      val blockHashBE =
        DoubleSha256DigestBE.fromBytes(ECPrivateKey.freshPrivateKey.bytes)
  ...
  it must "generate a range for a block filter query for the genesis block" in { chainHandler: ChainHandler =>
    val genesisHeader =
    ...
    val blockHeaderDb = {
      BlockHeaderDbHelper.fromBlockHeader(height = 0, chainWork = Pow.getBlockProof(genesisHeader), bh = genesisHeader)
    }
  ...
  it must "generate the correct range of block filters if a header is reorged" in { chainHandler: ChainHandler =>
    val reorgFixtureF = buildChainHandlerCompetingHeaders(chainHandler)
    ...
    val assert1F = for {
      ...
      newHeaderB <- newHeaderBF
      newHeaderC <- newHeaderCF
      blockHeaderBatchOpt <- chainHandler.nextBlockHeaderBatchRange(prevStopHash =
                                                                      ChainTestUtil.regTestGenesisHeaderDb.hashBE,
                                                                    batchSize = batchSize)
    } yield {
      assert(blockHeaderBatchOpt.isDefined)
      val marker = blockHeaderBatchOpt.get
      checkReorgHeaders(header1 = newHeaderB, header2 = newHeaderC, bestHash = marker.stopBlockHash.flip)
    ...
    for {
      ...
      blockHeaderBatchOpt <- chainApiD.nextBlockHeaderBatchRange(prevStopHash =
                                                                   ChainTestUtil.regTestGenesisHeaderDb.hashBE,
                                                                 batchSize = batchSize)
  ...
  it must "return None for ChainHandler.nextBlockHeaderBatchRange if we are synced" in { chainHandler: ChainHandler =>
    val genesisHeader =
      chainHandler.chainConfig.chain.genesisBlock.blockHeader
    val assert1F = for {
      rangeOpt <-
  ...
  it must "generate a range for a block filter header query" in { chainHandler: ChainHandler =>
    for {
  ...
  it must "read compact filters for the database" in { chainHandler: ChainHandler =>
    for {
      bestBlock <- chainHandler.getBestBlockHeader()
  ...
  it must "return the number of confirmations for the chain tip" in { chainHandler: ChainHandler =>
    for {
  ...
  it must "return the number of confirmations" in { chainHandler: ChainHandler =>
  ...
  it must "return none for the number of confirmations for a non-existent block" in { chainHandler: ChainHandler =>
    chainHandler.getNumberOfConfirmations(DoubleSha256DigestBE.empty).map { result =>
  ...
  // G -> A -> B
  // G -> C -> D -> E
  it must "return none for the number of confirmations for a reorged block" in { chainHandler: ChainHandler =>
    for {
      genesis <- chainHandler.getBestBlockHeader()
  ...
  it must "fail to return the height by block time" in { chainHandler: ChainHandler =>
    recoverToSucceededIf[RuntimeException] {
      for {
        bestBlock <- chainHandler.getBestBlockHeader()
  ...
  it must "fail to return the height by block stamp with an unknown hash" in { chainHandler: ChainHandler =>
  ...
  it must "find filters between heights" in { chainHandler: ChainHandler =>
    chainHandler.getFiltersBetweenHeights(0, 1).map { filters =>
      ...
      val genesisFilterResponse = FilterResponse(genesis.golombFilter, genesis.blockHashBE, genesis.height)

      assert(filters == Vector(genesisFilterResponse))
    }
  }

  it must "get the correct height from an epoch second" in { chainHandler: ChainHandler =>
    for {
      height <-
  ...
  it must "fail when processing duplicate filters" in { chainHandler: ChainHandler =>
  ...
  it must "check isMissingChainWork when there is over a 100 headers" in { chainHandler: ChainHandler =>
  ...
  it must "process a new valid block header with a callback" in { chainHandler: ChainHandler =>
