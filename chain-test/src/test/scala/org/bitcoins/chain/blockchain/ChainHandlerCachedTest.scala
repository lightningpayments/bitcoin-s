...
class ChainHandlerCachedTest extends ChainDbUnitTest {
  ...
  it must "throw an error when we have no chains" in { chainHandlerCached: ChainHandlerCached =>
  ...
  it must "get best filter header with zero blockchains in memory" in { chainHandlerCached: ChainHandlerCached =>
