...
class BitcoinSServerMainBitcoindTorTest extends BitcoinSAppConfigBitcoinFixtureNotStarted with CachedTor {

  behavior of "BitcoinSServerMain"

  it must "start our app server with bitcoind as a backend with tor" in { config: BitcoinSAppConfig =>
    ...
    for {
      ...
      balance = ConsoleCli.exec(CliCommand.GetBalance(isSats = true), cliConfig)
      addr = ConsoleCli.exec(CliCommand.GetNewAddress(labelOpt = None), cliConfig)
