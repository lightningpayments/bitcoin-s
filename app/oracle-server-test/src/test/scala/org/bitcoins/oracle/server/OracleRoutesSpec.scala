...
import org.bitcoins.core.protocol.tlv.{NormalizedString, OracleAnnouncementV0TLV, OracleAttestmentV0TLV}
...
class OracleRoutesSpec extends AnyWordSpec with ScalatestRouteTest with MockFactory {
  ...
  "The oracle server" must {
    ...
    "get public key" in {
      ...
      Get() ~> route ~> check {
        ...
        assert(responseAs[String] == s"""{"result":"${key.hex}","error":null}""")
      }
    }
    ...
    "get staking address" in {
      ...
      Get() ~> route ~> check {
        assert(contentType == `application/json`)
        assert(responseAs[String] == s"""{"result":"$testAddress","error":null}""")
      }
    }
    ...
    "list announcements" in {
      ...
      Get() ~> route ~> check {
        assert(contentType == `application/json`)
        assert(responseAs[String] == s"""{"result":["${dummyOracleEvent.eventName}"],"error":null}""")
      }
    }
    ...
    "get enum announcement" in {
      ...
      val route = oracleRoutes.handleCommand(ServerCommand("getannouncement", Arr(eventName)))
    ...
    "create enum announcement" in {
      ...
      val route =
        oracleRoutes.handleCommand(
          ServerCommand("createenumannouncement", Arr(Str("id"), Str("2021-02-04T00:00:00Z"), Arr(Str("1"), Str("2")))))

      Post() ~> route ~> check {
        assert(contentType == `application/json`)
        assert(responseAs[String] == s"""{"result":"${OracleAnnouncementV0TLV.dummy.hex}","error":null}""")
      }
    }
    ...
    "create enum announcement with just date" in {
      ...
      val route =
        oracleRoutes.handleCommand(
          ServerCommand("createenumannouncement", Arr(Str("id"), Str("2021-02-04"), Arr(Str("1"), Str("2")))))
    ...
    "create numeric announcement" in {
      (mockOracleApi
        .createNewDigitDecompAnnouncement(_: String, _: Instant, _: UInt16, _: Boolean, _: Int, _: String, _: Int32))
        .expects("id", Instant.ofEpochSecond(1612396800), UInt16(2), false, 17, "units", Int32.zero)
        .returning(Future.successful(OracleAnnouncementV0TLV.dummy))

      val route =
        oracleRoutes.handleCommand(
          ServerCommand("createnumericannouncement",
                        Arr(Str("id"), Str("2021-02-04T00:00:00Z"), Num(0), Num(131000), Str("units"), Num(0))))
    ...
    "create numeric announcement with just date" in {
      (mockOracleApi
        .createNewDigitDecompAnnouncement(_: String, _: Instant, _: UInt16, _: Boolean, _: Int, _: String, _: Int32))
        .expects("id", Instant.ofEpochSecond(1612396800), UInt16(2), true, 17, "units", Int32.zero)
        .returning(Future.successful(OracleAnnouncementV0TLV.dummy))

      val route =
        oracleRoutes.handleCommand(
          ServerCommand("createnumericannouncement",
                        Arr(Str("id"), Str("2021-02-04"), Num(-1), Num(131000), Str("units"), Num(0))))
      ...
      val route =
        oracleRoutes.handleCommand(
          ServerCommand("createdigitdecompannouncement",
                        Arr(Str("id"), Num(1612396800), Num(2), Bool(true), Num(17), Str("units"), Num(0))))
    ...
    "sign enum announcement" in {
      ...
      val route =
        oracleRoutes.handleCommand(ServerCommand("signenum", Arr(Str("id"), Str("outcome"))))

      Post() ~> route ~> check {
        assert(contentType == `application/json`)
        assert(responseAs[String] == s"""{"result":"${dummyAttestmentTLV.hex}","error":null}""")
      }
    }
    ...
    "sign numeric announcement" in {
      ...
      val route =
        oracleRoutes.handleCommand(ServerCommand("signdigits", Arr(Str("id"), Num(123))))
    ...
    "get signatures" in {
      ...
      val route =
        oracleRoutes.handleCommand(ServerCommand("getsignatures", Arr(Str("id"))))
    ...
    "sign message" in {
      ...
      val route =
        oracleRoutes.handleCommand(ServerCommand("signmessage", Arr(Str("message"))))

      Post() ~> route ~> check {
        assert(contentType == `application/json`)
        assert(responseAs[String] == s"""{"result":"${sig.hex}","error":null}""")
      }
    }
    ...
    "delete announcement" in {
      ...
      Post() ~> route ~> check {
        assert(contentType == `application/json`)
        assert(responseAs[String] == s"""{"result":"${dummyOracleEvent.announcementTLV.hex}","error":null}""")
      }
    }
    ...
    "backup" in {
      ...
      val route =
        oracleRoutes.handleCommand(ServerCommand("backuporacle", Arr(Str("/tmp/location"))))
    ...
    "get oracle name" in {
      ...
      Post() ~> route ~> check {
        assert(contentType == `application/json`)
        assert(responseAs[String] == s"""{"result":"oracle name","error":null}""")
      }
    }
    ...
    "set oracle name" in {
      ...
      val route =
        oracleRoutes.handleCommand(ServerCommand("setoraclename", Arr(Str("oracle name"))))
