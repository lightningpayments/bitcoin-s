...
import org.bitcoins.core.protocol.script.{MultiSignatureScriptPubKey, P2SHScriptPubKey, P2WSHWitnessSPKV0}
...
case class CoreRoutes()(implicit system: ActorSystem, config: BitcoinSAppConfig) extends ServerRoute {
  import system.dispatcher

  def handleCommand: PartialFunction[ServerCommand, Route] = {
    case ServerCommand("finalizepsbt", arr) =>
      withValidServerCommand(FinalizePSBT.fromJsArr(arr)) { case FinalizePSBT(psbt) =>
        complete {
          psbt.finalizePSBT
    ...
    case ServerCommand("extractfrompsbt", arr) =>
      withValidServerCommand(ExtractFromPSBT.fromJsArr(arr)) { case ExtractFromPSBT(psbt) =>
        complete {
    ...
    case ServerCommand("converttopsbt", arr) =>
      withValidServerCommand(ConvertToPSBT.fromJsArr(arr)) { case ConvertToPSBT(tx) =>
        complete {
    ...
    case ServerCommand("combinepsbts", arr) =>
      withValidServerCommand(CombinePSBTs.fromJsArr(arr)) { case CombinePSBTs(psbts) =>
        complete {
    ...
    case ServerCommand("decoderawtransaction", arr) =>
      withValidServerCommand(DecodeRawTransaction.fromJsArr(arr)) { case DecodeRawTransaction(tx) =>
        complete {
    ...
    case ServerCommand("decodepsbt", arr) =>
      withValidServerCommand(DecodePSBT.fromJsArr(arr)) { case DecodePSBT(psbt) =>
        complete {
    ...
    case ServerCommand("analyzepsbt", arr) =>
      withValidServerCommand(AnalyzePSBT.fromJsArr(arr)) { case AnalyzePSBT(psbt) =>
        complete {
          ...
          val optionalsJson: Vector[(String, Num)] = {
            val fee = psbt.feeOpt.map(fee => "fee" -> Num(fee.satoshis.toLong.toDouble))
            val vsize =
              psbt.estimateVSize.map(vsize => "estimated_vsize" -> Num(vsize.toDouble))
            val feeRate =
              psbt.estimateSatsPerVByte.map(feeRate => "estimated_sats_vbyte" -> Num(feeRate.toLong.toDouble))
    ...
    case ServerCommand("decodeoffer", arr) =>
      withValidServerCommand(DecodeOffer.fromJsArr(arr)) { case DecodeOffer(offerTLV) =>
        complete {
    ...
    case ServerCommand("decodecontractinfo", arr) =>
      withValidServerCommand(DecodeContractInfo.fromJsArr(arr)) { case DecodeContractInfo(contractInfo) =>
        complete {
          Server.httpSuccess(writeJs(contractInfo)(contractInfoV0TLVJsonWriter))
        }
      }

    case ServerCommand("decodeannouncement", arr) =>
      withValidServerCommand(DecodeAnnouncement.fromJsArr(arr)) { case DecodeAnnouncement(announcement) =>
        complete {
          Server.httpSuccess(writeJs(announcement)(oracleAnnouncementTLVJsonWriter))
        }
      }

    case ServerCommand("decodeattestments", arr) =>
      withValidServerCommand(DecodeAttestations.fromJsArr(arr)) { case DecodeAttestations(attestments) =>
        complete {
    ...
    case ServerCommand("createmultisig", arr) =>
      withValidServerCommand(CreateMultisig.fromJsArr(arr)) { case CreateMultisig(requiredKeys, keys, addressType) =>
        complete {
    ...
    case ServerCommand("zipdatadir", arr) =>
      withValidServerCommand(ZipDataDir.fromJsArr(arr)) { case ZipDataDir(path) =>
        complete {
