...
case class CompactFilterHeaderDAO()(implicit ec: ExecutionContext, override val appConfig: ChainAppConfig)
  ...
  class CompactFilterHeaderTable(tag: Tag) extends Table[CompactFilterHeaderDb](tag, schemaName, "cfheaders") {
    ...
    override def * = {
      (hash, filterHash, previousFilterHeader, blockHash, height).<>(CompactFilterHeaderDb.tupled,
  ...
  override def createAll(filterHeaders: Vector[CompactFilterHeaderDb]): Future[Vector[CompactFilterHeaderDb]] = {
    createAllNoAutoInc(ts = filterHeaders, database = safeDatabase)
  }

  /** Finds the rows that correlate to the given primary keys */
  override protected def findByPrimaryKeys(
      ids: Vector[DoubleSha256DigestBE]): Query[Table[CompactFilterHeaderDb], CompactFilterHeaderDb, Seq] =
  ...
  def getBetweenHeightsQuery(
      from: Int,
      to: Int): profile.StreamingProfileAction[Seq[CompactFilterHeaderDb], CompactFilterHeaderDb, Effect.Read] = {
