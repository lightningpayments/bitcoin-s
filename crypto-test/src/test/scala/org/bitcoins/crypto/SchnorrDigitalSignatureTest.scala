...
class SchnorrDigitalSignatureTest extends BitcoinSCryptoTest {
  ...
  it must "must create and verify a digital signature" in {
    forAll(NumberGenerator.bytevector(32), CryptoGenerators.privateKey) { case (bytes, privKey) =>
  ...
  it must "correctly compute signature points" in {
    forAll(CryptoGenerators.privateKey, NumberGenerator.bytevector(32)) { case (privKey, data) =>
  ...
  it must "correctly compute signature points for sigs with fixed nonces" in {
    forAll(CryptoGenerators.privateKey, NumberGenerator.bytevector(32), CryptoGenerators.privateKey) {
  ...
  it must "leak keys if two messages are signed" in {
    forAll(CryptoGenerators.nonZeroPrivKey,
           ...
           NumberGenerator.bytevector(32)) { case (privKey, nonce, message1, message2) =>
