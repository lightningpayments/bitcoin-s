...
import org.bitcoins.explorer.model.{CreateAnnouncementExplorer, CreateAttestations, Oracle, SbAnnouncementEvent}
import org.bitcoins.explorer.picklers.ExplorerPicklers
import org.bitcoins.tor.{Socks5ClientTransport, Socks5ProxyParams}
import play.api.libs.json.{JsArray, JsBoolean, JsError, JsNull, JsNumber, JsObject, JsString, JsSuccess, JsValue, Json}

import java.net.URI
import scala.concurrent.Future

/** A class that implements the Suredbits oracle explorer API */
case class SbExplorerClient(env: ExplorerEnv, proxyParams: Option[Socks5ProxyParams])(implicit system: ActorSystem) {
  import ExplorerPicklers._
  import system.dispatcher
  private val httpClient: HttpExt = Http(system)

  private val httpConnectionPoolSettings =
    Socks5ClientTransport.createConnectionPoolSettings(new URI(env.baseUri), proxyParams)
  ...
  def listAnnouncements(): Future[Vector[SbAnnouncementEvent]] = {
    ...
    responseF.flatMap { response =>
      ...
      result match {
        ...
        case err: JsError =>
          Future.failed(new RuntimeException(s"Failed to parse response for listAnnouncements, err=$err"))
      }
    }
  }
  ...
  /** Gets an announcement from the oracle explorer
    ...
    */
  def getAnnouncement(announcement: OracleAnnouncementTLV): Future[SbAnnouncementEvent] = {
  ...
  /** Gets an announcement from the oracle explorer
    * @see https://gist.github.com/Christewart/a9e55d9ba582ac9a5ceffa96db9d7e1f#get-event
    */
  def getAnnouncement(announcementHash: Sha256Digest): Future[SbAnnouncementEvent] = {
    val base = env.baseUri
    ...
    responseF.flatMap { response =>
      ...
      result match {
        case success: JsSuccess[SbAnnouncementEvent] =>
          Future.successful(success.value)
        case err: JsError =>
          Future.failed(new RuntimeException(s"Failed to parse response for getAnnouncement, err=$err"))
      }
    }
  }
  ...
  /** Creates an announcement on the oracle explorer
    ...
    */
  def createAnnouncement(oracleEventExplorer: CreateAnnouncementExplorer): Future[Unit] = {
    val base = env.baseUri
    ...
    val httpReq =
      HttpRequest(uri = uri,
                  ...
                  entity = HttpEntity(ContentTypes.`application/x-www-form-urlencoded`, string))
    val responseF = sendRequest(httpReq)
    responseF.map(_ => ())
  }
  ...
  def createAttestations(attestations: CreateAttestations): Future[Unit] = {
    val base = env.baseUri
    val uri = Uri(base + s"announcements/${attestations.announcementHash.hex}/attestations")
  ...
  private def sendRequest(httpReq: HttpRequest): Future[JsValue] = {
    ...
    responseJsonF.flatMap {
      case x @ (_: JsNumber | _: JsString | _: JsArray | JsNull | _: JsBoolean) =>
        Future.failed(new RuntimeException(s"Incorrect formatted response from oracle explorer, got=$x"))
      case obj: JsObject =>
        val map = obj.value
        val error = map.get("error")
        val result = map.get("result")
        if (error.get != JsNull) {
          Future.failed(new RuntimeException(s"Error returned by oracle explroer, err=${error}"))
        } else {
