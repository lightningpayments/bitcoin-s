...
class DLCClient(
  ...
  override def receive: Receive = LoggingReceive { case DLCClient.Connect(peer) =>
    ...
    case Tcp.Connected(peerOrProxyAddress, _) =>
      ...
      peer.socks5ProxyParams match {
        ...
        case None =>
          ...
          val _ =
            context.actorOf(Props(new DLCConnectionHandler(dlcWalletApi, connection, handlerP, dataHandlerFactory)))
          connectedAddress.foreach(_.success(peerAddress))
      }
  }

  def socks5Connecting(proxy: ActorRef, remoteAddress: InetSocketAddress, proxyAddress: InetSocketAddress): Receive =
    LoggingReceive {
      case c @ Tcp.CommandFailed(_: Socks5Connect) =>
        val ex = c.cause.getOrElse(new IOException(s"UnknownError: $c"))
        log.error(s"connection failed to $remoteAddress via SOCKS5 $proxyAddress", ex)
        throw ex
      case Socks5Connected(_) =>
        log.info(s"connected to $remoteAddress via SOCKS5 proxy $proxyAddress")
        val _ = context.actorOf(Props(new DLCConnectionHandler(dlcWalletApi, proxy, handlerP, dataHandlerFactory)))
...
object DLCClient {
  ...
  def connect(
    ...
    val actor =
      system.actorOf(props(dlcWalletApi, Some(promise), handlerP, dataHandlerFactory))
    actor ! Connect(peer)
    promise.future
  }
}
