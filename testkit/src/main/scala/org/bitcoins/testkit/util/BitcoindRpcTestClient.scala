...
case class BitcoindRpcTestClient(override val binary: Path, version: BitcoindVersion)(implicit system: ActorSystem)
  ...
  private lazy val bitcoindInstance: BitcoindInstanceLocal = {
    BitcoindRpcTestUtil.getInstance(bitcoindVersion = version, binaryDirectory = binaryDirectory)
  }
  ...
  override def start(): Future[BitcoindRpcClient] = {
    clientOpt match {
      ...
      case None =>
        val clientF =
          BitcoindRpcTestUtil.startedBitcoindRpcClient(Some(bitcoindInstance), clientAccum = Vector.newBuilder)
        clientF.map { c =>
          clientOpt = Some(c)
          c
        }
  ...
  override def stop(): Future[BitcoindRpcClient] = {
    clientOpt match {
      ...
      case None =>
        Future.failed(new RuntimeException(s"BitcoindRpcClient was not defined!"))
    }
  }
