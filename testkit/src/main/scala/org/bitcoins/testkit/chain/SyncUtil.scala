...
import org.bitcoins.chain.blockchain.sync.{ChainSync, FilterSync, FilterWithHeaderHash}
...
abstract class SyncUtil extends Logging {
  ...
  def getBlockHeaderFunc(bitcoind: BitcoindRpcClient)(implicit
      ec: ExecutionContext): DoubleSha256DigestBE => Future[BlockHeader] = { hash: DoubleSha256DigestBE =>
    bitcoind.getBlockHeader(hash).map(_.blockHeader)
  }

  /** Creates a function that you can pass a block header to and it's return's it's [[GolombFilter]] */
  def getFilterFunc(bitcoind: BitcoindV19RpcClient, filterType: FilterType)(implicit
      ec: ExecutionContext): BlockHeader => Future[FilterWithHeaderHash] = { case header: BlockHeader =>
  ...
  def getNodeApi(bitcoindRpcClient: BitcoindRpcClient)(implicit ec: ExecutionContext): NodeApi = {
    new NodeApi {
      ...
      override def downloadBlocks(blockHashes: Vector[DoubleSha256Digest]): Future[Unit] = {
        ...
        val f: Vector[DoubleSha256Digest] => Future[Vector[Unit]] = { case hashes =>
        ...
        val batchedExecutedF =
          FutureUtil.batchExecute(elements = blockHashes, f = f, init = Vector.empty, batchSize = batchSize)

        batchedExecutedF.map { _ =>
          logger.info(s"Done fetching ${blockHashes.length} hashes from bitcoind")
          ()
        }
      }
    }
  ...
  def getNodeApiWalletCallback(bitcoindRpcClient: BitcoindRpcClient, walletF: Future[Wallet])(implicit
      ...
      override def downloadBlocks(blockHashes: Vector[DoubleSha256Digest]): Future[Unit] = {
        ...
        val batchedExecutedF = {
          for {
            wallet <- walletF
            updatedWallet <-
              FutureUtil.batchExecute[DoubleSha256Digest, Wallet](elements = blockHashes,
                                                                  f = f,
                                                                  init = wallet,
                                                                  batchSize = batchSize)
          } yield updatedWallet
  ...
  def getNodeChainQueryApiWalletCallback(bitcoind: BitcoindRpcClient, walletF: Future[Wallet])(implicit
  ...
  /** Syncs the given chain handler to the given bitcoind node.
    ...
    */
  def syncBitcoindWithChainHandler(bitcoindWithChainHandler: BitcoindChainHandlerViaRpc)(implicit
      ec: ExecutionContext): Future[BitcoindBaseVersionChainHandlerViaRpc] = {
    val getBestBlockHash = getBestBlockHashFunc(bitcoindWithChainHandler.bitcoindRpc)
    val getBlockHeader = getBlockHeaderFunc(bitcoindWithChainHandler.bitcoindRpc)

    val chainApiF = ChainSync.sync(bitcoindWithChainHandler.chainHandler, getBlockHeader, getBestBlockHash)
    for {
      chainApi <- chainApiF
    } yield BitcoindBaseVersionChainHandlerViaRpc(bitcoindRpc = bitcoindWithChainHandler.bitcoindRpc,
  ...
  /** Syncs the given chain handler to the given bitcoind node. This also syncs block filters
    ...
    */
  def syncBitcoindV19WithChainHandler(bitcoindWithChainHandler: BitcoindV19ChainHandler)(implicit
      ec: ExecutionContext,
    ...
    } yield BitcoindV19ChainHandler(bitcoindRpc = bitcoindWithChainHandler.bitcoindRpc,
