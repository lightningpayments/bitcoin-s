...
import org.bitcoins.testkit.rpc.{BitcoindRpcTestUtil, CachedBitcoind, CachedBitcoindNewest, CachedBitcoindV19}
...
/** Bitcoin-s wallet test trait that uses cached bitcoinds
  ...
  */
trait BitcoinSWalletTestCachedBitcoind extends BitcoinSFixture with BaseWalletTest with EmbeddedPg {
  _: CachedBitcoind[_] =>
  ...
  def withFundedWalletAndBitcoindCached(
      ...
      bitcoind: BitcoindRpcClient)(implicit walletAppConfig: WalletAppConfig): FutureOutcome = {
    val builder: () => Future[WalletWithBitcoind] = { () =>
      for {
        walletWithBitcoind <- createWalletWithBitcoindCallbacks(bitcoind = bitcoind,
    ...
    makeDependentFixture[WalletWithBitcoind](builder,
  ...
  def withFundedWalletAndBitcoind(test: OneArgAsyncTest, bip39PasswordOpt: Option[String])(implicit
    ...
    val resultF = for {
      bitcoind <- bitcoindF
      outcome = withFundedWalletAndBitcoindCached(test, bip39PasswordOpt, bitcoind)
      f <- outcome.toFuture
    } yield f
  ...
  private def stopBitcoind(bitcoindF: Future[BitcoindRpcClient]): Future[Unit] = {
    for {
      bitcoind <- bitcoindF
...
trait BitcoinSWalletTestCachedBitcoindNewest extends BitcoinSWalletTestCachedBitcoind with CachedBitcoindNewest {
...
trait BitcoinSWalletTestCachedBitcoinV19 extends BitcoinSWalletTestCachedBitcoind with CachedBitcoindV19 {
  ...
  def withFundedWalletAndBitcoindCachedV19(
      test: OneArgAsyncTest,
      bip39PasswordOpt: Option[String],
      bitcoind: BitcoindV19RpcClient)(implicit walletAppConfig: WalletAppConfig): FutureOutcome = {
    val builder: () => Future[WalletWithBitcoindV19] = { () =>
      for {
        walletWithBitcoind <- createWalletWithBitcoindCallbacks(bitcoind = bitcoind,
                                                                bip39PasswordOpt = bip39PasswordOpt)
        walletWithBitcoindV19 = WalletWithBitcoindV19(walletWithBitcoind.wallet, bitcoind)
        fundedWallet <- fundWalletWithBitcoind[WalletWithBitcoindV19](walletWithBitcoindV19)
        _ <- SyncUtil.syncWalletFullBlocks(wallet = fundedWallet.wallet, bitcoind = bitcoind)
        _ <- BitcoinSWalletTest.awaitWalletBalances(fundedWallet)
      } yield fundedWallet
    }
  ...
  def withNewWalletAndBitcoindCachedV19(
    ...
    val builder: () => Future[WalletWithBitcoind] = composeBuildersAndWrap(
      ...
      dependentBuilder = { (bitcoind: BitcoindV19RpcClient) =>
        BitcoinSWalletTest.createWalletWithBitcoindV19(bitcoind, bip39PasswordOpt)
      },
      wrap = (_: BitcoindRpcClient, walletWithBitcoind: WalletWithBitcoindV19) => walletWithBitcoind
    )
  ...
  override def withFixture(test: OneArgAsyncTest): FutureOutcome = {
    val f: Future[Outcome] = for {
      ...
      futOutcome = withFundedWalletAndBitcoindCachedV19(test, getBIP39PasswordOpt(), bitcoind)(getFreshWalletAppConfig)
